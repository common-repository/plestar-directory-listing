<?php
require_once( PDL_PATH . 'includes/class-listing.php' );

/**
 * @since 5.0
 */
function pdl_save_listing( $args = array(), $error = false, $context = '' ) {
    ...
    $args = apply_filters( 'pdl_save_listing_args', $args, $error, $context );
    ...
    $listing['post_status']  = ! empty( $args['post_status'] ) ? $args['post_status'] : ( $listing['listing_id'] ? pdl_get_option( 'edit-post-status' ) : 'pending' );

    // Fields.
    $listing['fields'] = ! empty( $args['fields'] ) ? $args['fields'] : array();
    foreach ( array_keys( $listing['fields'] ) as $field_id ) {
        $field_obj   = pdl_get_form_field( $field_id );
    ...
    if ( ! empty( $listing['post_title'] ) && empty( $listing['post_name'] ) ) {
        ...
        if ( 'csv-import' == $context ) {
            $post_name_hash = 'pdl-slug-' . sha1( $listing['post_name'] );
    ...
    if ( empty( $listing['post_author'] ) ) {
        ...
        //         if ( 0 == $post_author ) {
        //             // Create user.
        //             if ( $email_field = pdl_get_form_fields( array( 'validators' => 'email', 'unique' => 1 ) ) ) {
    ...
    $listing = apply_filters( 'pdl_save_listing_data', $listing, $context );
    extract( $listing );
    ...
    if ( $sequence_id ) {
        update_post_meta( $listing_id, '_pdl[import_sequence_id]', $sequence_id );
    }

    $listing_obj = pdl_get_listing( $listing_id );
    ...
    // friends. See #2945.
    do_action_ref_array( 'PDL_Listing::set_field_values', array( &$listing_obj, $fields ) );    
    ...
    // Force GUIDs to always be <home-url>?post_type=pdl_listing&p=<post_id>
    if ( $adding && ( ! isset( $guid ) || ! $guid ) ) {
        $post_link = add_query_arg( array( 'post_type' => PDL_POST_TYPE, 'p' => $listing_id ), '' );
 ...
 * @since 5.0
 */
function pdl_get_listing( $listing_id ) {
    return PDL_Listing::get( $listing_id );
}

/**
 * @since 5.0.6
 */
function pdl_get_listings_by_email( $email ) {
    global $wpdb;
    ...
    if ( $user = get_user_by( 'email', $email ) ) {
        ...
        $post_ids = array_merge(
            $post_ids, 
            $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} WHERE post_type = %s AND post_status != %s AND post_author = %d", PDL_POST_TYPE, 'auto-draft', $user_id ) )
        );
    }
    ...
    if ( $email_field = pdl_get_form_fields( 'validators=email&unique=1' ) ) {
        ...
        $post_ids = array_merge(
            $post_ids,
            $wpdb->get_col( $wpdb->prepare( "SELECT DISTINCT post_id FROM {$wpdb->postmeta} WHERE meta_key = %s AND LOWER(meta_value) = %s", '_pdl[fields][' . $field_id . ']', strtolower( $email ) ) )
        );
    }

    // Filter everything through get_posts().
    $post_ids = get_posts( array( 'post_type' => PDL_POST_TYPE, 'post_status' => array( 'publish', 'draft', 'pending' ), 'posts_per_page' => -1, 'post__in' => $post_ids ? $post_ids : array( -1 ), 'fields' => 'ids' ) );
