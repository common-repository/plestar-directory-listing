...
final class PDL__Fee_Plan {
    ...
    public function save( $fire_hooks = true ) {
        ...
        if ( $fire_hooks ) {
            do_action_ref_array( 'pdl_fee_before_save', array( $this ) );
        }
        ...
        $update = $this->exists();
        if ( $update ) {
            $saved = $wpdb->update( $wpdb->prefix . 'pdl_plans', $row, array( 'id' => $this->id ) );
        } else {
            $saved = $wpdb->insert( $wpdb->prefix . 'pdl_plans', $row );
        ...
        if ( $saved ) {
            if ( $fire_hooks ) {
                do_action( 'pdl_fee_save', $this, $update );
            }

            $wpdb->update(
                $wpdb->prefix . "pdl_listings",
    ...
    public function delete() {
        global $wpdb;
        return $wpdb->delete( $wpdb->prefix . 'pdl_plans', array( 'id' => $this->id ) );
    }
    ...
    public function get_feature_list() {
        ...
        if ( pdl_get_option( 'allow-images' ) ) {
            if ( ! $this->images )
                $items['images'] = _x( 'No images allowed.', 'fee plan', 'PDM' );
            else
                $items['images'] = sprintf( _nx( '%d image allowed.', '%d images allowed.', $this->images, 'fee plan', 'PDM' ), $this->images );
        }

        $items = apply_filters( 'pdl_plan_feature_list', $items, $this );
        return $items;
    }
    ...
    public static function get_instance( $fee_id ) {
        ...
        $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_plans WHERE id = %d", $fee_id ), ARRAY_A );
    ...
    private function validate() {
        ...
        if ( ! $this->label ) {
            $errors['label'] = _x( 'Fee label is required.', 'fees-api', 'PDM' );
        }
        ...
        if ( $this->days > 3650 ) {
            $errors['days'] = _x( 'Fee listing duration must be a number less than 10 years (3650 days).', 'fees-api', 'PDM' );
        }

        if ( 1 == $this->recurring ) {
            if ( 0 == $this->days ) {
                $errors[] = str_replace( '<a>', '<a href="#pdl-fee-form-days">', _x( 'To set this fee as "Recurring" you must have a time for the listing to renew (e.g. 30 days). To avoid issues with the listing, please edit the <a>fee plan</a> appropriately.', 'fees-api', 'PDM' ) );
            }

            $error_message = _x( 'To set this fee as "Recurring" you must set a price for your fee plan. To avoid issues with the listing, please edit the <a>fee plan</a> appropriately.', 'fees-api', 'PDM' );

            if ( 'flat' == $this->pricing_model && 0 == $this->amount ) {
                $errors[] = str_replace( '<a>', '<a href="#pdl-fee-form-fee-price">', $error_message );
            }

            if ( 'variable' == $this->pricing_model && 0 == array_sum( $this->pricing_details ) ) {
                $errors[] = str_replace( '<a>', '<a href="#pdl-fee-form-fee-category">', $error_message );
            }
...
require_once( PDL_INC . 'compatibility/deprecated/class-fee-plan.php' );
