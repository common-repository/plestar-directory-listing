...
class PDL_Themes {
    ...
    function __construct() {
        ...
        $this->template_dirs[] = trailingslashit( get_stylesheet_directory() ) . 'directory-listing/';

        if ( get_stylesheet_directory() != get_template_directory() )
            $this->template_dirs[] = trailingslashit( get_template_directory() ) . 'directory-listing/';
        ...
        $this->template_dirs[] = trailingslashit( PDL_PATH . 'templates' );
        ...
        add_filter( 'pdl_form_field_display', array( &$this, 'field_theme_override' ), 999, 4 );
        add_action( 'wp_footer', array( $this, 'fee_specific_coloring' ), 999 );

        if ( is_admin() ) {
            require_once( PDL_PATH . 'includes/admin/class-themes-admin.php' );
            $this->admin = new PDL_Themes_Admin( $this, pdl()->licensing );
        }
    }
    ...
    function call_theme_function( $fname, $args = array() ) {
        ...
        $alternatives = array( 'pdl_themes__' . $theme_name . '_' . $fname,
                               'pdl_' . $theme_name . '_' . $fname,
                               $theme_name . '_' . $fname );
    ...
    function load_theme_translation() {
        ...
        $mofile = untrailingslashit( $theme->path ) . "/languages/pdl-{$theme->id}-{$locale}.mo";

        if ( ! file_exists( $mofile ) )
            return;

        return load_textdomain( 'pdl-' . $theme->id, $mofile );
    }
    ...
    function enqueue_theme_scripts() {
        ...
        if ( 'theme' == pdl_get_option( 'themes-button-style' ) && file_exists( $theme->path . 'assets/buttons.css' ) ) {
            wp_enqueue_style(
                $theme->id . '-buttons',
    ...
    function fee_specific_coloring() {
        $plans = pdl_get_fee_plans( array( 'include_free' => true, 'tag' => '' ) );
        ...
        foreach ( $plans as $plan ) {
            ...
            echo '.pdl-listing-excerpt.pdl-listing-plan-id-' . $plan->id . '{';
    ...
    function get_themes_dir() {
        return WP_CONTENT_DIR . '/plestardirectory-themes/';
    }

    function get_themes_directories() {
        $res = array();

        $res[ PDL_PATH . 'themes/' ] = PDL_URL . 'themes/';
        $res[ WP_CONTENT_DIR . '/plestardirectory-themes/' ] = WP_CONTENT_URL . '/plestardirectory-themes/';
    ...
    function find_themes( $reload = false ) {
        ...
        foreach ( $this->get_themes_directories() as $path => $url ) {
            $dirs = PDL_FS::ls( $path, 'filter=dir' );
    ...
    function set_active_theme( $theme_id = '' ) {
        ...
        $ok = update_option( 'pdl-active-theme', $theme_id );
    ...
    function get_active_theme() { 
        $active = get_option( 'pdl-active-theme', 'default' );
        $themes = $this->get_installed_themes();
    ...
    function missing_suggested_fields( $key = '' ) {
        ...
        $current_fields_tags = $wpdb->get_col( "SELECT tag FROM {$wpdb->prefix}pdl_form_fields" );

        $missing_tags = array_diff( $suggested_fields, $current_fields_tags );

        foreach ( $missing_tags as $mt ) {
            $info = $pdl->formfields->get_default_fields( $mt );
...
/*            array( 'assets/css', 'array/string', array() ),
    ...
    public function sync_settings() {
        return;
        $themes_data = get_option( 'pdl-themes-licenses', array() );
        $pdl_settings = get_option( 'pdl_settings', array() );

        if ( $themes_data ) {
            $changed = false;
            foreach ( $themes_data as $theme_id => $theme_data ) {
                if ( ! array_key_exists( 'license-key-theme-' . $theme_id, $pdl_settings ) ) {
                    $changed = true;
                    $pdl_settings[ 'license-key-theme-' . $theme_id ] = $theme_data['license'];
                    update_option( 'pdl-license-status-theme-' . $theme_id, $theme_data['status'] );
                }
            }

            if ( $changed ) {
                update_option( 'pdl_settings', $pdl_settings );
            }
        }
    ...
    function add_theme_data() {
        foreach ( $this->themes as &$t ) {
            ...
            if ( ! $t->is_core_theme ) {
                pdl()->licensing->add_item( array(
                    'item_type' => 'theme',
                    'id'        => $t->id,
                    'name'      => $t->name,
                    'version'   => $t->version,
                ) );
                // $t->license_key = pdl_get_option( 'license-key-theme-' . $t->id );
                // $t->license_status = get_option( 'pdl-license-status-theme-' . $t->id );
            }
    ...
    private function _get_official_themes() {
        $official_themes = get_transient( 'pdl-official-themes' );
        ...
        $url = add_query_arg( $params, 'http://plestar.net/edd-api/v2/products/' );

        $response = wp_remote_get( $url, array( 'timeout' => 15, 'sslverify' => false ) );

        if ( is_wp_error( $response ) ) {
            set_transient( 'pdl-official-themes', array(), HOUR_IN_SECONDS );
            return array();
        }
        ...
        set_transient( 'pdl-official-themes', $official_themes, WEEK_IN_SECONDS );
     ...
     *
     * - Inside Plestar Directory Listing directory.
     * - On Plestar Directory Listing Themes directory.
     */
    ...
    private function render_template_file( $template_id, $path, $vars = array() ) {
        ...
        $html = apply_filters( 'pdl_x_render', $html, $template_id, $vars );
        return $html;
    }
     ...
     *
     * @link http://docs.plestar.net/themes/customization.html#block-and-block-variable-customization
     *
     * @since 5.0
     *
    ...
    private function get_template_meta( $template_path ) {
        ...
        $template_meta = get_file_data( $template_path, $default_headers, 'plestar_directory_template' );
...
'_bar_items' => array( 'links', 'search' ),*/
        ...
        $vars = apply_filters( 'pdl_template_variables', $vars, $template_id );
        $vars = apply_filters( 'pdl_template_variables__' . $template_id, $vars, $path );
    ...
    function install_theme( $file ) {
        $themes_dir = wp_normalize_path( WP_CONTENT_DIR . '/plestardirectory-themes/' ); // TODO: do not hardcode this directory.
        list( $temp_dir, $unzipped_dir, ) = PDL_FS::unzip_to_temp_dir( $file );
        $package_dir = $unzipped_dir;

        // Search for a dir containing 'theme.json'.
        $files = PDL_FS::ls( $unzipped_dir, 'recursive=1' );
        foreach ( $files as $f ) {
        ...
        if ( ! file_exists( PDL_FS::join( $package_dir, 'theme.json' ) ) ) {
            PDL_FS::rmdir( $temp_dir );
            return new WP_Error( 'no-theme-file',
                                 _x( 'ZIP file is not a valid PDL theme file.', 'themes', 'PDM' ) );
        }

        if ( ! PDL_FS::mkdir( $themes_dir ) ) {
            PDL_FS::rmdir( $temp_dir );
            return new WP_Error( 'no-themes-directory',
                                 _x( 'Could not create themes directory.', 'themes', 'PDM' ) );
        }

        $dest_dir = $themes_dir . basename( $package_dir );

        if ( ! PDL_FS::rmdir( $dest_dir ) ) {
            PDL_FS::rmdir( $temp_dir );
            return new WP_Error( 'old-theme-not-removed',
                                 sprintf( _x( 'Could not remove previous theme directory "%s".', 'themes', 'PDM' ), 
                                          $dest_dir ) );
        }

        if ( ! PDL_FS::movedir( $package_dir, $themes_dir ) ) {
            PDL_FS::rmdir( $temp_dir );
            return new WP_Error( 'theme-not-copied', _x( 'Could not move new theme into theme directory.', 'themes', 'PDM' ) );
        }
...
function pdl_x_render( $template_id, $vars = array(), $wrapper = '' ) {
    global $pdl;

    if ( $wrapper && ! isset( $vars['_wrapper'] ) )
        $vars['_wrapper'] = $wrapper;

    return $pdl->themes->render( $template_id, $vars );
}

function pdl_x_render_page( $template_id, $vars = array() ) {
    return pdl_x_render( $template_id, $vars, 'page' );
}

function pdl_x_part( $template_id, $vars = array() ) {
    global $pdl;
    echo $pdl->themes->render_part( $template_id, $vars );
}

function pdl_add_template_dir( $dir_or_file ) {
    global $pdl;
    return $pdl->themes->add_template_dir( $dir_or_file );
}
