 ...
 * @since 3.5.3
 */
class PDL_Listings_Widget extends WP_Widget {
    ...
    public function __construct( $name, $description = '' ) {
        ...
        $this->defaults['title'] = str_replace( array( 'PDL', '_' ), array( '', ' '), get_class( $this ) );
    ...
    public function form( $instance ) {
        ...
        printf( '<p><label for="%s">%s</label> <input id="%s" name="%s" type="text" value="%s" size="5" /></p>',
                ...
                _x( 'Number of listings to display:', 'widgets', 'PDM' ),
                $this->get_field_id( 'number_of_listings' ),
        ...
        if ( in_array( 'images', $this->supports ) ) {
            echo '<h4>';
            _ex( 'Thumbnails', 'widgets', 'PDM' );
            echo '</h4>';

            printf( '<p><input id="%s" class="pdl-toggle-images" name="%s" type="checkbox" value="1" %s /> <label for="%s">%s</label></p>',
                    $this->get_field_id( 'show_images' ),
                    $this->get_field_name( 'show_images' ),
                    $this->get_field_value( $instance, 'show_images' ) ? 'checked="checked"' : '',
                    $this->get_field_id( 'show_images' ),
                    _x( 'Show thumbnails', 'widgets', 'PDM' ) );
            ...
            _ex( 'Image width (in px):', 'widgets', 'PDM' );
            echo '</label> ';
            ...
            echo '<br /><span class="help">' . _x( 'Leave blank for automatic width.', 'widgets', 'PDM' ) . '</span>';
            echo '</p>';
    ...
    public function widget( $args, $instance ) {
        ...
        if ( ! $out ) {
            if ( $listings = $this->get_listings( $instance ) ) {
                ...
                $out .= '<ul class="pdl-listings-widget-list">';
