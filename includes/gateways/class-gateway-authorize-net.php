 ...
 * @since 3.5.7
 */
class PDL__Gateway__Authorize_Net extends PDL__Payment_Gateway {
    ...
    public function __construct() {
        ...
        add_action( 'pdl_listing_expired', array( $this, 'maybe_handle_expiration' ) );
    }
    ...
    public function get_title() {
        return _x( 'Authorize.net', 'authorize-net', 'PDM' );
    }
    ...
    public function get_settings() {
        return array(
            array( 'id' => 'login-id', 'name' => __( 'Login ID', 'authorize-net', 'PDM' ), 'type' => 'text' ),
            array( 'id' => 'transaction-key', 'name' => __( 'Transaction Key', 'authorize-net', 'PDM' ), 'type' => 'text' )
        );
    }
    ...
    public function validate_settings() {
        ...
        if ( ! $login_id )
            $errors[] = _x( 'Login ID is missing.', 'authorize-net', 'PDM' );

        if ( ! $trans_key )
            $errors[] = _x( 'Transaction Key is missing.', 'authorize-net', 'PDM' );
    ...
    public function process_payment( $payment ) {
        ...
        if ( $response->approved || $response->held ) {
            ...
            if ( $response->held ) {
                $error_msg = sprintf( _x( 'Payment is being held for review by the payment gateway. The following reason was given: "%s".', 'authorize-net', 'PDM' ),
            ...
            $error_msg = sprintf( _x( 'The payment gateway didn\'t accept the credit card or billing information. The following reason was given: "%s".', 'authorize-net', 'PDM' ),
                         '(' . $response->response_reason_code . ') ' . rtrim( $response->response_reason_text, '.' ) );
            $payment->log( $error_msg );
            $payment->save();
        ...
        $error_msg = sprintf( _x( 'Payment was rejected. The following reason was given: "%s".', 'authorize-net', 'PDM' ),
                                  '(' . $response->response_reason_code . ') ' . rtrim( $response->response_reason_text, '.' ) );
    ...
    private function process_payment_recurring( $payment ) {
        ...
        $subscription_args = array(
            ...
            'creditCardCardNumber' => sanitize_text_field($_POST['card_number']),
            'creditCardExpirationDate' => sprintf( '%02d', $_POST['exp_month'] ) . '-' . substr( sanitize_text_field($_POST['exp_year']), 2 ),
            'creditCardCardCode' => intval($_POST['cvc']),
            'billToFirstName' => sanitize_text_field($_POST['payer_first_name']),
            'billToLastName' => sanitize_text_field($_POST['payer_last_name']),
            'billToAddress' => sanitize_text_field($_POST['payer_address']),
            'billToCity' => sanitize_text_field($_POST['payer_city']),
            'billToState' => sanitize_text_field($_POST['payer_state']),
            'billToCountry' => sanitize_text_field($_POST['payer_country']),
            'billToZip' => sanitize_text_field($_POST['payer_zip']),
            'customerEmail' => sanitize_email($_POST['payer_email']),
        ...
        if ( ! class_exists( 'AuthorizeNetARB' ) )
            require_once( PDL_PATH . 'vendors/anet_php_sdk/AuthorizeNet.php' );
        ...
        if ( ! $response->isOk() ) {
            $error_msg = sprintf( _x( 'Payment failed. Reason: %s', 'authorize-net', 'PDM' ), $response->getMessageText() );
            $payment->log( $error_msg );
    ...
    private function generate_subscription_name( $payment ) {
        $listing = pdl_get_listing( $payment->listing_id );
        $recurring_item = $payment->find_item( 'recurring_plan' );

        $name  = '';
        $name .= $listing->get_title() ? $listing->get_title() : sprintf( _x( 'Listing #%d', 'authorize-net', 'PDM' ), $listing->get_id() );
        $name .= ' - ';
    ...
    private function setup_webhooks() {
        ...
        $webhook_id = get_option( 'pdl-authorize-webhook-id', '' );
        ...
        if ( ! $webhook_id ) {
            ...
            update_option( 'pdl-authorize-webhook-id', $response->webhookId );
        }
    }
