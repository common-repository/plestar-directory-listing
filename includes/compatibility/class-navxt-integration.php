...
class PDL_NavXT_Integration {
    ...
    function main_page_breadcrumb( $trail ) {
        ...
        $trail->add( new bcn_breadcrumb( get_the_title( pdl_get_page_id() ),
                                         '',
                                         array(),
                                         pdl_get_page_link(),
                                         pdl_get_page_id() ) );
     ...
     *
     * TODO: Can we replace pdl_current_listing_id with this?
    ...
    private function get_current_listing_id() {
        ...
        if ( $id_or_slug ) {
            $listing_id = pdl_get_post_by_id_or_slug( $id_or_slug, 'id', 'id' );
        } else {
    ...
    function before_tag( $trail ) {
        $tag = get_term_by( 'slug', get_query_var( 'tag' ), PDL_TAGS_TAX );

        if ( ! $tag )
            return;

        global $wp_query;
        $term = get_term( $category_id, PDL_CATEGORY_TAX );
        $this->state['queried'] = $wp_query->get_queried_object();
    ...
    function before_submit( $trail ) {
        $trail->add( new bcn_breadcrumb( _x( 'Submit Listing', 'navxt', 'PDM' ) ) );
    }

    function before_edit( $trail ) {
        $trail->add( new bcn_breadcrumb( _x( 'Edit Listing', 'navxt', 'PDM' ) ) );
    }

    function before_search( $trail ) {
        $trail->add( new bcn_breadcrumb( _x( 'Search', 'navxt', 'PDM' ) ) );
    }
