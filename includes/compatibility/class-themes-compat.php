...
class PDL__Themes_Compat {
    ...
    public function __construct() {
        ...
        add_action( 'pdl_after_dispatch', array( $this, 'add_workarounds' ) );
    }
    ...
    public function get_themes_with_fixes() {
        ...
        return apply_filters( 'pdl_themes_with_fixes_list', $themes_with_fixes );
    }
    ...
    public function theme_genesis() {
        $is_listings_view = in_array( pdl_current_view(), array( 'all_listings', 'show_listing' ), true );

        if ( ! $is_listings_view && ! pdl_is_taxonomy() ) {
            return;
        }
    ...
    public function theme_divi() {
        ...
        if ( 'et_full_width_page' != get_post_meta( pdl_get_page_id( 'main' ), '_et_pb_page_layout', true ) ) {
            return;
        }
    ...
    //     // pdl_debug_e( pdl_current_view_output() );
    // }
    //
