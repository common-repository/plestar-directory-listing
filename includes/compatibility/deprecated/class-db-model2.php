<?php
/**
 * @deprecated since next-release. Use {@link PDL__DB__Model} instead.
 */
class PDL_DB_Model2 {
    ...
    protected function update_timestamps( $row ) {
        global $wpdb;
        $table = $wpdb->prefix . 'pdl_' . $this->table;
    ...
    protected static function _find( $id, $args = array(), $table = '', $classname = '' ) {
        ...
        if ( $order ) {
            $order_field = pdl_starts_with( $order, '-' ) ? substr( $order, 1 ) : $order;
            $order_dir = pdl_starts_with( $order, '-' ) ? 'DESC' : 'ASC';
