<?php
require_once ( PDL_PATH . 'includes/compatibility/deprecated.php' );
...
class PDL_Compat {
    ...
    function __construct() {
        ...
        if ( pdl_get_option( 'disable-cpt' ) ) {
            ...
            require_once( PDL_PATH . 'includes/compatibility/class-cpt-compat-mode.php' );
            $nocpt = new PDL__CPT_Compat_Mode();
        } else {
            require_once( PDL_PATH . 'includes/compatibility/class-themes-compat.php' );
            new PDL__Themes_Compat();
        }
    ...
    function load_integrations() {
        if ( isset( $GLOBALS['sitepress'] ) ) {
            require_once( PDL_PATH . 'includes/compatibility/class-wpml-compat.php' );
            $wpml_integration = new PDL_WPML_Compat();
        }

        if ( function_exists( 'bcn_display' ) ) {
            require_once( PDL_PATH . 'includes/compatibility/class-navxt-integration.php' );
            $navxt_integration = new PDL_NavXT_Integration();
        }

        if ( class_exists( 'Advanced_Excerpt' ) ) {
            require_once( PDL_PATH . 'includes/compatibility/class-advanced-excerpt-integration.php' );
            $advanced_excerpt_integration = new PDL_Advanced_Excerpt_Integration();
        }

        if ( defined( 'CUSTOM_PERMALINKS_PLUGIN_VERSION') ) {
            require_once( PDL_PATH . 'includes/compatibility/class-custom-permalinks-integration.php' );
            $custom_permalinks_integration = new PDL_Custom_Permalink_Integration();
        }
    }
    ...
    function workarounds_for_wp_bugs() {
        ...
        add_filter( 'pdl_query_clauses', array( &$this, '_fix_pagination_issue' ), 10, 2 );
    }
