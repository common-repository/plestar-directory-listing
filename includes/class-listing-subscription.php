<?php
class PDL__Listing_Subscription {
    ...
    private function fill_data_from_db( $key, $val ) {
        ...
        $row = $wpdb->get_row( $wpdb->prepare( "SELECT listing_id, subscription_id, subscription_data FROM {$wpdb->prefix}pdl_listings WHERE {$key} = %s", $val ) );
    ...
    public function get_parent_payment() {
        if ( $this->parent_payment_id ) {
            return pdl_get_payment( $this->parent_payment_id );
        }
    ...
    public function record_payment( $args_or_payment = array() ) {
        ...
        if ( is_array( $args_or_payment ) ) {
            ...
            if ( ! empty( $args['gateway_tx_id'] ) ) {
                ...
                $payment = PDL_Payment::objects()->get( array( 'gateway_tx_id' => $p_id, 'gateway' => $p_gateway ) ); // Just in case the payment is already in the database.
            }

            if ( ! $payment ) {
                $payment = new PDL_Payment( $args_or_payment );
            }
        } else {
    ...
    public function renew() {
        $listing = pdl_get_listing( $this->listing_id );
        $listing->update_plan();
        $listing->set_status( 'complete' );
        $listing->set_post_status( 'publish' );

        do_action( 'pdl_listing_renewed', $listing );
    }
    ...
    public function cancel() {
        ...
        do_action( 'pdl_listing_subscription_canceled', $this->listing_id );
    }
    ...
    private function save() {
        ...
        return $wpdb->update( $wpdb->prefix . 'pdl_listings', $row, array( 'listing_id' => $this->listing_id ) );
    }
}
