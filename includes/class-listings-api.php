...
if ( ! class_exists( 'PDL_Listings_API' ) ) {

/**
 * @since 3.5.4
 */
class PDL_Listings_API {

    public function __construct() {
        add_action( 'pdl_payment_completed', array( $this, 'update_listing_after_payment' ) );
    }
    ...
    public function update_listing_after_payment( $payment ) {
        ...
        if ( $is_renewal) {
            pdl_insert_log( array( 'log_type' => 'listing.renewal', 'object_id' => $payment->listing_id, 'message' => _x( 'Listing renewed', 'listings api', 'PDM' ) ) );
            $listing->set_post_status( 'publish' );

            do_action( 'pdl_listing_renewed', $listing, $payment );
        }
    }
    ...
    private function get_quick_search_fields() {
        ...
        foreach ( pdl_get_option( 'quick-search-fields', array() ) as $field_id ) {
     ...
     * @uses PDL_ListingsAPI::get_quick_search_fields().
    ...
    public function quick_search( $keywords, $location = false ) {
        ...
        require_once( PDL_PATH . 'core/helpers/class-search-helper.php' );
        ...
        $helper = new PDL__Search_Helper( $args );
    ...
    public function get_thumbnail_id( $listing_id ) {
        if ( $listing = pdl_get_listing( $listing_id ) ) {
