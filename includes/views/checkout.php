<?php
class PDL__Views__Checkout extends PDL__View {
    ...
    public function enqueue_resources() {
        foreach ( pdl()->payment_gateways->get_available_gateways() as $gateway ) {
    ...
    public function dispatch() {
        ...
        if ( $this->is_successful_initial_payment() ) {
            ...
            $this->_redirect( add_query_arg( $args, pdl_url( 'submit_listing' ) ) );
        }
        ...
        if ( has_action( 'pdl_checkout_before_action' ) ) {
            ...
            do_action( 'pdl_checkout_before_action', $checkout );
        ...
        $vars['invoice'] = pdl()->payments->render_invoice( $this->payment );
        $vars['chosen_gateway'] = $this->gateway;
        $vars['checkout_form_top'] = pdl_capture_action( 'pdl_checkout_form_top', $this->payment );
        $vars['checkout_form'] = $this->checkout_form();
        $vars['checkout_form_bottom'] = pdl_capture_action( 'pdl_checkout_form_bottom', $this->payment );
        $vars['payment'] = $this->payment;
        $vars['nonce'] = wp_create_nonce( 'pdl-checkout-' . $this->payment->id );
    ...
    private function pre_dispatch() {
        if ( ! pdl()->payment_gateways->can_pay() )
            wp_die( _x( 'Can not process a payment at this time. Please try again later.', 'checkout', 'PDM' ) );
    ...
    private function fetch_payment() {
        if ( ! $this->payment_id && ! empty( $_REQUEST['payment'] ) ) {
            $this->payment = PDL_Payment::objects()->get( array( 'payment_key' => $_REQUEST['payment'] ) );
        } elseif ( $this->payment_id ) {
            $this->payment = PDL_Payment::objects()->get( $this->payment_id );
        }
    ...
    private function validate_nonce() {
        ...
        if ( ! wp_verify_nonce( $nonce, 'pdl-checkout-' . $this->payment_id ) )
            wp_die( _x( 'Invalid nonce received.', 'checkout', 'PDM' ) );
    }
    ...
    private function set_current_gateway() {
        ...
        if ( ! empty( $_REQUEST['gateway'] ) ) {
            ...
            $gateway_ids = array_keys( pdl()->payment_gateways->get_available_gateways( array( 'currency_code' => $this->payment->currency_code ) ) );
            $chosen_gateway = array_shift( $gateway_ids );
        }

        if ( ! pdl()->payment_gateways->can_use( $chosen_gateway ) ) {
            wp_die( _x( 'Invalid gateway selected.', 'checkout', 'PDM' ) );
        }

        $this->gateway = pdl()->payment_gateways->get( $chosen_gateway );
        if ( ! $this->gateway->supports_currency( $this->payment->currency_code ) )
            wp_die( _x( 'Selected gateway does not support payment\'s currency.', 'checkout', 'PDM' ) );
    }

    private function checkout_form() {
        $checkout_form  = '';
        // $checkout_form .= pdl_capture_action( 'pdl_checkout_form_top', $this->payment );
        $checkout_form .= $this->gateway->render_form( $this->payment, $this->errors );
        // $checkout_form .= pdl_capture_action( 'pdl_checkout_form_bottom', $this->payment );
        $checkout_form .= '<div class="pdl-checkout-submit"><input type="submit" value="' . _x( 'Pay Now', 'checkout', 'PDM' ) . '" /></div>';
    ...
    private function do_checkout() {
        ...
        $validation_errors = apply_filters( 'pdl_checkout_validation_errors', $validation_errors, $this->payment );
        ...
        if ( ! empty( $res['error'] ) ) {
            ...
            $this->errors[] = _x( 'Unknown gateway error.', 'checkout', 'PDM' );
        }
    ...
    private function handle_return_request() {
        if ( ! $this->gateway ) {
            wp_die( __( 'There was an error trying to process your request. No gateway is selected.', 'checkout', 'PDM' ) );
        }
