...
class PDL__Views__Submit_Listing extends PDL__Authenticated_Listing_View {
    ...
    public function get_title() {
        return _x( 'Submit A Listing', 'views', 'PDM' );
    }
    ...
    public function enqueue_resources() {
        ...
        wp_enqueue_script(
            'pdl-submit-listing',
            PDL_URL . 'assets/js/submit-listing.min.js',
            array(),
            PDL_VERSION
        );

        wp_enqueue_script( 'pdl-checkout' );
        ...
        if ( 'disabled' != pdl_get_option( 'create-account-during-submit-mode' ) ) {
            wp_enqueue_script( 'password-strength-meter' );
        }

        wp_localize_script( 'pdl-submit-listing', 'pdlSubmitListingL10n', array(
            'categoriesPlaceholderTxt' => _x( 'Click this field to add categories', 'submit listing', 'PDM' ),
            'completeListingTxt' => _x( 'Complete Listing', 'submit listing', 'PDM' ),
            'continueToPaymentTxt' => _x( 'Continue to Payment', 'submit listing', 'PDM' ),
            'isAdmin' => current_user_can( 'administrator' )
        ) );

        do_action( 'pdl_submit_listing_enqueue_resources' );
    }
    ...
    public function dispatch() {
        ...
        if ( ! $this->can_submit( $msg ) )
            return pdl_render_msg( $msg );

        if ( $this->editing ) {
            $message = '';

            if ( empty( $_REQUEST['listing_id'] ) ) {
                $message = _x( "No listing ID was specified.", 'submit listing', 'PDM' );
            } elseif ( ! pdl_user_can( 'edit', $_GET['listing_id'] ) ) {
                $message = _x( "You can't edit this listing.", 'submit listing', 'PDM' );
            }

            if ( $message ) {
                return pdl_render_msg( $message );
            }
        }
        ...
        if ( ! empty( $_POST ) && ! empty( $_POST['reset'] ) && 'reset' == $_POST['reset'] ) {
            if ( ! $this->editing ) {
                ...
                return $this->_redirect( pdl_url( 'submit_listing' ) );
            }

            return $this->_redirect( pdl_url( 'edit_listing', $this->listing->get_id() ) );
        }

        if ( ! $this->editing && 'auto-draft' != get_post_status( $this->listing->get_id() ) ) {
            $possible_payment = PDL_Payment::objects()->filter( array( 'listing_id' => $this->listing->get_id(), 'payment_type' => 'initial', 'status' => 'pending' ) )->get();
            ...
            if ( current_user_can( 'administrator' ) ) {
                return pdl_render_msg(
                    str_replace(
                        ...
                        _x( 'This listing can\'t be edited at this time because it has no fee plan associated. Please <a>edit the listing</a> on the backend and associate it to a fee plan.', 'submit listing', 'PDM' )
                    ),
                    'error'
                );
            } else {
                return pdl_render_msg( _x( 'This listing can\'t be edited at this time. Please try again later or contact the admin if the problem persists.', 'submit listing', 'PDM' ), 'error' );
            }
        }
        ...
        if ( current_user_can( 'administrator' ) )
            $this->messages( _x( 'You\'re logged in as admin, payment will be skipped.', 'submit listing', 'PDM' ), 'notice', 'general' );

        $instructions = trim( pdl_get_option( 'submit-instructions' ) );
        if ( $instructions ) {
        ...
        foreach ( $this->messages as $context => $items ) {
            ...
            foreach ( $items as $i ) {
                $messages[ $context ] .= sprintf( '<div class="pdl-msg %s">%s</div>', $i[1], $i[0] );
            }
        }

        $html = pdl_render( 'submit-listing',
    ...
    public function ajax_reset_plan() {
        ...
        if ( ! $this->editing ) {
            ...
            $this->data['previous_categories'] = wp_get_post_terms( $this->listing->get_id(), PDL_CATEGORY_TAX, array( 'fields' => 'ids' ) );
        ...
        if( ! empty( $_POST['listingfields'] ) ) {
        	$_POST['listingfields'] = array_map('sanitize_text_field',$_POST['listingfields']);
            update_post_meta( $this->listing->get_id(), '_pdl_temp_listingfields', $_POST['listingfields'] );
        }

        wp_set_post_terms( $this->listing->get_id(), array(), PDL_CATEGORY_TAX, false );
    ...
    private function configure() {
        $this->available_plans = pdl_get_fee_plans();
    ...
    public function ajax_sections() {
        ...
        foreach ( $this->sections as $section ) {
            ...
            foreach ( $messages as $i ) {
                $messages_html .= sprintf( '<div class="pdl-msg %s">%s</div>', $i[1], $i[0] );
            }

            $sections[ $section['id'] ] = $section;
            $sections[ $section['id'] ]['html'] = pdl_render( 'submit-listing-section', array( 'section' => $section, 'messages' => $messages_html ) );
        }
    ...
    private function can_submit( &$msg = null ) {
        ...
        if ( 'submit_listing' == pdl_current_view() && pdl_get_option( 'disable-submit-listing' ) ) {
            if ( current_user_can( 'administrator' ) )
                $msg = _x( '<b>View not available</b>. Do you have the "Disable Frontend Listing Submission?" setting checked?', 'templates', 'PDM' );
            else
                $msg = _x( 'View not available.', 'templates', 'PDM' );
    ...
    private function find_or_create_listing() {
        ...
        if ( ! empty( $_REQUEST['listing_id'] ) && false != get_post_status( $_REQUEST['listing_id'] ) ) {
            ...
            $listing_id = wp_insert_post( array( 'post_author' => get_current_user_id(), 'post_type' => PDL_POST_TYPE, 'post_status' => 'auto-draft', 'post_title' => '(no title)' ) );
    ...
    private function submit_sections() {
        ...
        if ( $this->can_edit_plan_or_categories() ) {
            $sections['plan_selection'] = array(
                'title' => $this->skip_plan_selection ? _x( 'Category selection', 'submit listing', 'PDM' ) : _x( 'Category & plan selection', 'submit listing', 'PDM' )
            );
        }

        $sections['listing_fields'] = array(
            'title' => _x( 'Listing Information', 'submit listing', 'PDM' ) );

        if ( pdl_get_option( 'allow-images' ) ) {
            $sections['listing_images'] = array(
                'title' => _x( 'Listing Images', 'submit listing', 'PDM' )
            );
        }

        if ( ! $this->editing && ! pdl_get_option( 'require-login' ) && 'disabled' != pdl_get_option( 'create-account-during-submit-mode' ) && ! is_user_logged_in() ) {
            $sections['account_creation'] = array(
                'title' => _x( 'Account Creation', 'submit listing', 'PDM' )
            );
        }

        $sections = apply_filters( 'pdl_submit_sections', $sections, $this );

        if ( ! $this->editing && pdl_get_option( 'display-terms-and-conditions' ) ) {
            $sections['terms_and_conditions'] = array(
                'title' => _x( 'Terms and Conditions', 'submit listing', 'PDM' )
            );
        }
    ...
    private function prepare_sections() {
        foreach ( $this->sections as &$section ) {
            $callback = PDL_Utils::normalize( $section['id'] );

            if ( ! $this->listing->has_fee_plan() && 'plan_selection' != $section['id'] ) {
                $section['flags'][] = 'collapsed';
                $section['flags'][] = 'disabled';
                $section['html'] = _x( '(Please choose a fee plan above)', 'submit listing', 'PDM' );
            ...
            $section = apply_filters( 'pdl_submit_section_' . $section['id'], $section, $this );

            $section['flags'][] = $section['state'];
        }

        $this->sections = apply_filters( 'pdl_submit_prepare_sections', $this->sections, $this );
    }
    ...
    private function section_render( $template, $vars = array(), $result = true ) {
        ...
        $vars['_submit'] = $this;
        $output = pdl_render( $template, $vars, false );
    ...
    private function plan_selection() {
        ...
        $category_field = pdl_get_form_fields( 'association=category&unique=1' ) or die( '' );

        if ( ! $plans )
            wp_die( _x( 'Can not submit a listing at this moment. Please try again later.', 'submit listing', 'PDM' ) );
        ...
        if ( $this->editing ) {
            ...
            if ( ! $categories && ! empty( $_POST ) ) {
                ...
                $this->messages( _x( 'Please select a category.', 'submit listing', 'PDM' ), 'error', 'plan_selection' );
            }
        } else {
            $categories = $category_field->value_from_POST();
            ...
            if ( $this->skip_plan_selection ) {
                ...
                if ( $this->saving() && ! $categories ) {
                    $this->messages( _x( 'Please select a category for your listing.', 'submit listing', 'PDM' ), 'error', 'plan_selection' );
                    $this->prevent_save = true;
                }
            } else {
        ...
        if ( $categories && ! $category_field->validate( $categories, $errors ) ) {
            ...
            $plan = pdl_get_fee_plan( $plan_id );

            if ( ! $plan || ! $plan->enabled || ! $plan->supports_category_selection( $categories ) ) {
                if ( $this->editing ) {
                    $this->messages( _x( 'Please choose a valid category for your plan.', 'submit listing', 'PDM' ), 'error', 'plan_selection' );
                } else {
                    $this->messages( _x( 'Please choose a valid fee plan for your category selection.', 'submit listing', 'PDM' ), 'error', 'plan_selection' );
                }

                $this->prevent_save = true;
            } else {
                // Set categories.
                wp_set_post_terms( $this->listing->get_id(), $categories, PDL_CATEGORY_TAX, false );
            ...
            wp_set_post_terms( $this->listing->get_id(), $current_categories, PDL_CATEGORY_TAX, false );
        }
    ...
    private function listing_fields() {
        $fields = pdl_get_form_fields( array( 'association' => '-category' ) );
        $fields = apply_filters_ref_array( 'pdl_listing_submit_fields', array( &$fields, &$this->listing ) );
        $saved_listingfields = get_post_meta( $this->listing->get_id(), '_pdl_temp_listingfields', true );
        ...
        foreach ( $fields as $field ) {
            ...
            if ( ! empty( $_POST['save_listing'] ) ) {
                $field_errors = null;
                $validate_res = apply_filters_ref_array( 'pdl_listing_submit_validate_field', array(
        ...
        // FIXME: fake this (for compatibility with modules) until we move everything to pdl_save_listing() and
        // friends. See #2945.
        do_action_ref_array( 'PDL_Listing::set_field_values', array( &$this->listing, $field_values ) );

        if ( $validation_errors ) {
            $this->messages( _x( 'Something went wrong. Please check the form for errors, correct them and submit again.', 'listing submit', 'PDM' ), 'error', 'listing_fields' );
            $this->prevent_save = true;
        }
    ...
    private function listing_images() {
        ...
        //
        //     update_post_meta( $img_id, '_pdl_image_weight', ! empty( $updated_meta['order'] ) ? intval( $updated_meta['order'] ) : 0 );
        //     update_post_meta( $img_id, '_pdl_image_caption', ! empty( $updated_meta['caption'] ) ? trim( $updated_meta['caption'] ) : '' );
        // }
        ...
        if ( ! empty( $_POST['save_listing'] ) && ! count( $images_meta ) && pdl_get_option( 'enforce-image-upload' ) ) {
            $this->prevent_save = true;
            $this->messages( _x( 'Image upload is required, please provide at least one image and submit again.', 'listing submit', 'PDM' ), 'error', 'listing_images' );
        }
        ...
        $image_min_file_size = intval( pdl_get_option( 'image-min-filesize' ) );
        $image_min_file_size = $image_min_file_size ? size_format( $image_min_file_size * 1024 ) : '0';

        $image_max_file_size = intval( pdl_get_option( 'image-max-filesize' ) );
        $image_max_file_size = $image_max_file_size ? size_format( $image_max_file_size * 1024 ) : '0';

        $image_min_width = intval( pdl_get_option( 'image-min-width' ) );
        $image_max_width = intval( pdl_get_option( 'image-max-width' ) );
        $image_min_height = intval( pdl_get_option( 'image-min-height' ) );
        $image_max_height = intval( pdl_get_option( 'image-max-height' ) );
    ...
    private function account_creation() {
        $mode = pdl_get_option( 'create-account-during-submit-mode' );
        $form_create = empty( $_POST['create-account'] ) ? false : ( $_POST['create-account'] == 'create-account' );
        $form_username = ! empty( $_POST['user_username'] ) ? sanitize_text_field(trim( $_POST['user_username']) ) : '';
        $form_email = ! empty( $_POST['user_email'] ) ? sanitize_email(trim( $_POST['user_email']) ) : '';
        $form_password = ! empty( $_POST['user_password'] ) ? sanitize_text_field($_POST['user_password']) : '';
        ...
        if ( ( $this->saving() && 'required' == $mode ) || $form_create ) {
            ...
            if ( ! $form_username ) {
                $this->messages( _x( 'Please enter your desired username.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
                $error = true;
            }

            if ( ! $error && ! $form_email ) {
                $this->messages( _x( 'Please enter the e-mail for your new account.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
                $error = true;
            }

            if ( ! $error && ! $form_password ) {
                $this->messages( _x( 'Please enter the password for your new account.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
                $error = true;
            }

            // if ( ! $error && ( ! $form_password_retype  || ( $form_password != $form_password_retype ) ) ) {
            //     $this->messages( _x( 'Passwords entered do not match.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
            //     $error = true;
            // }

            if ( ! $error && $form_username && username_exists( $form_username ) ) {
                $this->messages( _x( 'The username you chose is already in use. Please use a different one.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
                $error = true;
            }

            if ( ! $error && $form_email && email_exists( $form_email ) ) {
                $this->messages( _x( 'The e-mail address you chose for your account is already in use.', 'submit listing', 'PDM' ), 'error', 'account_creation' );
                $error = true;
            }
        ...
        if ( 'optional' == $mode ) {
            $html .= '<input id="pdl-submit-listing-create_account" type="checkbox" name="create-account" value="create-account" ' . checked( true, $form_create, false ) . '/>';
            $html .= '<label for="pdl-submit-listing-create_account">' . _x( 'Create a user account on this site', 'submit listing', 'PDM' ) . '</label>';
        }

        $html .= '<div id="pdl-submit-listing-account-details" class="' . ( ( 'optional' == $mode && ! $form_create ) ? 'pdl-hidden' : '' ) . '">';

        if ( 'required' == $mode ) {
            $html .= '<p>';
            $html .= _x( 'You need to create an account on the site. Please fill out the form below.', 'submit listing', 'PDM' );
            $html .= '</p>';
        }

        $html .= '<div class="pdl-form-field pdl-form-field-type-textfield">';
        $html .= '<div class="pdl-form-field-label">';
        $html .= '<label for="user_username">' . _x( 'Username:', 'submit listing', 'PDM' ) . '</label>';
        $html .= '</div>';
        $html .= '<div class="pdl-form-field-inner">';
        $html .= '<input id="pdl-submit-listing-user_username" type="text" name="user_username" value="' . esc_attr( $form_username ) .'" />';
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="pdl-form-field pdl-form-field-type-textfield">';
        $html .= '<div class="pdl-form-field-label">';
        $html .= '<label for="user_email">' . _x( 'Email:', 'submit listing', 'PDM' ) . '</label>';
        $html .= '</div>';
        $html .= '<div class="pdl-form-field-inner">';
        $html .= '<input id="pdl-submit-listing-user_email" type="text" name="user_email" value="' . esc_attr( $form_email ) . '" />';
        $html .= '</div>';
        $html .= '</div>';

        $html .= '<div class="pdl-form-field pdl-form-field-type-password">';
        $html .= '<div class="pdl-form-field-label">';
        $html .= '<label for="pdl-submit-listing-user_password">' . _x( 'Password:', 'submit listing', 'PDM' ) . '</label>';
        $html .= '</div>';
        $html .= '<div class="pdl-form-field-inner">';
        $html .= '<input id="pdl-submit-listing-user_password" type="password" name="user_password" value="" />';
        $html .= '<span class="pdl-password-strength-meter"></span>';
        $html .= '</div>';
        $html .= '</div>';

        // $html .= '<div class="pdl-form-field pdl-form-field-type-password">';
        // $html .= '<div class="pdl-form-field-label">';
        // $html .= '<label for="pdl-submit-listing-user_password_retype">' . _x( 'Password (type again):', 'submit listing', 'PDM' ) . '</label>';
        // $html .= '</div>';
        // $html .= '<div class="pdl-form-field-inner">';
        // $html .= '<input id="pdl-submit-listing-user_password_retype" type="password" name="user_password_retype" value="" />';
        // $html .= '</div>';
        // $html .= '</div>';
    ...
    private function terms_and_conditions() {
        $tos = trim( pdl_get_option( 'terms-and-conditions' ) );

        if ( ! $tos )
            return false;

        $is_url = pdl_starts_with( $tos, 'http://', false ) || pdl_starts_with( $tos, 'https://', false );
        $accepted = ! empty( $_POST['terms-and-conditions-agreement'] ) && 1 == $_POST['terms-and-conditions-agreement'];

        if ( $this->saving() && ! $accepted ) {
            $this->messages( _x( 'Please agree to the Terms and Conditions.', 'templates', 'PDM' ), 'error', 'terms_and_conditions' );
            $this->prevent_save = true;
        }
        ...
        if ( ! $is_url ) {
            $html .= '<label>';
            $html .= _x( 'Terms and Conditions:', 'templates', 'PDM' );
            $html .= '</label><br />';
            $html .= sprintf( '<textarea readonly="readonly" class="pdl-submit-listing-tos">%s</textarea>', esc_textarea( $tos ) );
            $html .= '<br />';
        }
        ...
        $label = _x( 'I agree to the <a>Terms and Conditions</a>', 'templates', 'PDM' );
        if ( $is_url )
    ...
    private function save_listing() {
        if ( ! $this->editing ) {
            ...
            // $extra = pdl_capture_action_array( 'pdl_listing_form_extra_sections', array( &$this->state ) );
            // return $this->render( 'extra-sections', array( 'output' => $extra ) );
            // do_action_ref_array( 'pdl_listing_form_extra_sections_save', array( &$this->state ) );
        ...
        $this->listing->set_post_status( $this->editing ? pdl_get_option( 'edit-post-status' ) : pdl_get_option( 'new-post-status' ) );
        ...
        delete_post_meta( $this->listing->get_id(), '_pdl_temp_listingfields' );
    ...
    private function done() {
        ...
        return pdl_render( 'submit-listing-done', $params );
    }
