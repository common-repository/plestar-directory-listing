 ...
 * @since 4.0
 */
class PDL__WordPress_Template_Integration {
    ...
    public function template_include( $template ) {
        ...
        global $post;
        if ( empty( $wp_query->pdl_view ) && ( ! isset( $post ) || ! $post instanceof WP_Post ) )
            return $template;
    ...
    private function get_template_alternatives() {
        ...
        $main_page_id = pdl_get_page_id( 'main' );
    ...
    public function maybe_spoof_post() {
        ...
        if ( ! $wp_query->is_main_query() || ! $wp_query->pdl_our_query )
            return;
    ...
    public function add_basic_body_classes( $classes = array() ) {
        if ( 'theme' == pdl_get_option( 'themes-button-style' ) ) {
            $classes[] = 'pdl-with-button-styles';
        }
    ...
    public function add_advanced_body_classes( $classes = array() ) {
        ...
        // really dispatched through PDL.
        $view = pdl_current_view();

        if ( ! $view )
            return $classes;

        $classes[] = 'directory-listing';
        $classes[] = 'pdl-view-' . $view;

        if ( $theme = wp_get_theme() ) {
            $classes[] = 'pdl-wp-theme-' . $theme->get_stylesheet();
            $classes[] = 'pdl-wp-theme-' . $theme->get_template();
        }

        if ( pdl_is_taxonomy() ) {
            $classes[] = 'pdl-view-taxonomy';
        }

        $classes[] = 'pdl-theme-' . $pdl->themes->get_active_theme();
    ...
    public function post_class( $classes, $more_classes, $post_id ) {
        ...
        if ( $post && 0 == $post->ID && $post_id == $post->ID ) {
            $classes[] = 'pdl-view-content-wrapper';
        }
    ...
    private function restore_things() {
        ...
        if ( ! isset( $wp_query->pdl_spoofed ) || ! $wp_query->pdl_spoofed )
            return;
    ...
    public function _comments_template($template) {
        $is_single_listing = is_single() && get_post_type() == PDL_POST_TYPE;
        $is_main_page = get_post_type() == 'page' && get_the_ID() == pdl_get_page_id( 'main' );
        ...
        // disable comments in PDL pages or if comments are disabled for listings
        if ( ( $is_single_listing && ! $comments_allowed ) || $is_main_page ) {
            return PDL_TEMPLATES_PATH . '/empty-template.php';
        }
    ...
    public function _category_template($template) {
        if (get_query_var(PDL_CATEGORY_TAX) && taxonomy_exists(PDL_CATEGORY_TAX)) {
            return pdl_locate_template(array('plestardirectory-category', 'wpdirlist-category'));
        }
    ...
    public function _single_template($template) {
        if (is_single() && get_post_type() == PDL_POST_TYPE) {
            return pdl_locate_template(array('plestardirectory-single', 'wpdirlist-single'));
        }
