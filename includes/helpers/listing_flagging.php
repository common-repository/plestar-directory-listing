 ...
 * @since 5.1.6
 */
class PDL__Listing_Flagging {

    public static function get_flagging_options() {
        $flagging_options = pdl_get_option( 'listing-flagging-options' );
    ...
    public static function is_flagged( $listing_id ) {
        return ( 1 == get_post_meta( $listing_id, '_pdl_flagged', true ) );
    }
    ...
    public static function get_flagging_meta( $listing_id, $key = 'all' ) {
        if ( ! $listing_id ) {
            return new WP_Error( 'missing_data', _x( 'Listing ID is required to save a report', 'flag listing', 'PDM' ) );
        }

        $flagging_data = get_post_meta( $listing_id,  '_pdl_flagged_data', true );
    ...
    public static function add_flagging( $listing_id, $data ) {
        ...
        if ( ! $data[ 'user_id' ] && ! $data[ 'ip' ] ) {
            return new WP_Error( 'missing_data', _x( 'User ID or IP address is required to save a report', 'flag listing', 'PDM' ) );
        }

        if ( empty( $data[ 'reason' ] ) || empty( $data[ 'comments' ] ) ) {
            return new WP_Error( 'missing_data', _x( 'Report reason or comment is required to save a report', 'flag listing', 'PDM' ) );
        }
        ...
        update_post_meta( $listing_id, '_pdl_flagged_data', $flagging_data );
        update_post_meta( $listing_id, '_pdl_flagged', 1 );

        do_action( 'pdl_listing_maybe_flagging_notice', PDL_Listing::get( $listing_id ), $data );
    ...
    public static function clear_flagging( $listing_id ) {
        delete_post_meta( $listing_id, '_pdl_flagged_data' );
        update_post_meta( $listing_id,  '_pdl_flagged', 0 );
    }
