...
class PDL_Email {
    ...
    public function __construct() {
        $this->content_type = pdl_get_option( 'listing-email-content-type', 'html' );
    }

    public function pdl_email_config ( &$phpmailer ) {
    ...
    public function send($format='both') {
        ...
        if ( $this->template ) {
            if ( $html_ = pdl_render( $this->template, array( 'subject' => $this->subject,
                'body' => $this->html ) ) ) {
                $this->html = $html_;
            }
        }
        ...
        add_action( 'phpmailer_init', array( $this, 'pdl_email_config' ), 10 );
        $result = wp_mail( $this->to, $this->subject, $message, $headers );
        remove_action( 'phpmailer_init', array( $this, 'pdl_email_config' ), 10 );
