 ...
 * @since 4.0
 */
class PDL_Field_Display_List implements IteratorAggregate {
    ...
    public function append( &$f ) {
        ...
        if ( $f instanceof PDL_Lightweight_Field_Display_Item ) {
            $this->items[ $f->id ] = $f;
        ...
        $this->displayed_fields[] = $field_id;
        $this->items[ $field_id ] = new PDL_Lightweight_Field_Display_Item( $f, $this->listing_id, $this->display );
    ...
    public function __get( $key ) {
        ...
        if ( 'html' == $key ) {
            ...
            if ( 'listing' == $this->display ) {
                $html = apply_filters( 'pdl_single_listing_fields', $html, $this->listing_id );
            }
        ...
        pdl_debug( 'Invalid field key: ' . $key );
        return new PDL_NoopObject(); // FIXME: templates shouldn't rely on a field existing.
        return false;
    }
 ...
 * @since 4.0
 */
class PDL_Lightweight_Field_Display_Item {
