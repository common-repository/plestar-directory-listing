...
final class PDL_FS {
    ...
    static function temp_dir() {
        ...
        $dir = $sys_tmp . uniqid( 'pdl' ) . DIRECTORY_SEPARATOR;
    ...
    static function unzip( $zipfile, $destdir ) {
        ...
        if ( ! wp_is_writable( $destdir ) ) {
            return new WP_Error( 'dest-not-writable',
                                 sprintf( _x( 'Destination dir "%s" is not writable.', 'fs helper', 'PDM' ), $destdir ) );
        }

        $normalized = basename( basename( strtolower( $zipfile ), '.zip' ), '.tmp' );
        $normalized = str_replace( array( 'directory-listing-', 'plestardirectory' ), '', $normalized );
