 ...
 * @since 5.0
 */
class PDL__Payment_Gateways {

    private $gateways = array();


    public function __construct() {
        add_action( 'pdl_modules_loaded', array( $this, 'load_gateways' ) );
        add_action( 'pdl_loaded', array( $this, '_execute_listener' ) );
        add_action( 'pdl_register_settings', array( $this, '_add_gateway_settings' ) );
        add_action( 'pdl_admin_notices', array( $this, '_admin_warnings' ) );
    }
    ...
    public function load_gateways() {
        ...
        require_once( PDL_PATH . 'includes/gateways/class-gateway-authorize-net.php' );
        $gateways[] = new PDL__Gateway__Authorize_Net();

        // Allow modules to add gateways.
        $gateways = apply_filters( 'pdl_payment_gateways', $gateways );
    ...
    public function _execute_listener() {
        $listener_id = ! empty( $_GET['pdl-listener'] ) ? $_GET['pdl-listener'] : '';
    ...
    public function _add_gateway_settings( $api ) {
        foreach ( $this->gateways as $gateway ) {
            pdl_register_settings_group( 'gateway_' . $gateway->get_id(), $gateway->get_title(), 'payment', array( 'desc' => $gateway->get_settings_text() ) );
            pdl_register_setting( array(
                'id' => $gateway->get_id(),
                'name' => sprintf( _x( 'Enable %s?', 'payment-gateways', 'PDM' ), $gateway->get_title() ),
                'type' => 'checkbox',
                'default' => false,
    ...
    public function _admin_warnings() {
        if ( empty( $_GET['page'] ) || 'pdl_settings' != $_GET['page'] )
            return;
        ...
        foreach ( $this->gateways as $gateway ) {
            if ( $gateway->is_enabled( true ) ) {
                ...
                $msg  = _x( 'The <gateway> gateway is enabled but not properly configured. The gateway won\'t be available until the following problems are fixed: <problems>.', 'payment-gateways', 'PDM' );
                $msg .= '<br />';
                $msg .= _x( 'Please check the <link>payment settings</link>.', 'payment-gateways', 'PDM' );

                $msg = str_replace( '<gateway>', '<b>' . $gateway->get_title() .'</b>', $msg );
                $msg = str_replace( '<problems>', '<b>' . $errors . '</b>', $msg );
                $msg = str_replace( array( '<link>', '</link>' ), array( '<a href="' . admin_url( 'admin.php?page=pdl_settings&tab=payment' ) . '">', '</a>' ), $msg );
        ...
        if ( ! $at_least_one_gateway ) {
            $msg = _x( 'You have payments turned on but no gateway is active and properly configured. Go to <link>Manage Options - Payment</link> to change the payment settings. Until you change this, the directory will operate in <i>Free Mode</i>.', 'payment-gateways', 'PDM' );
            $msg = str_replace( array( '<link>', '</link>' ), array( '<a href="' . admin_url( 'admin.php?page=pdl_settings&tab=payment' ) . '">', '</a>' ), $msg );
            pdl_admin_message( $msg, 'error' );
        }
    }
