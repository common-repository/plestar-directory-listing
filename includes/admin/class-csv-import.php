...
/**
 * Replaces `PDL_CSVImporter` (from 2.1) and adds support for sequential imports.
 * @since 3.5.8
 */
class PDL_CSV_Import {
    ...
    private function maybe_convert_encoding( $line ) {
        ...
        if ( isset( $this->settings['encoding'] ) ) {
            ...
            $encoding = pdl_detect_encoding( $line );
        }
    ...
    private function restore_state( $state_id ) {
        ...
        $csv_imports_dir = rtrim( $upload_dir['basedir'], DIRECTORY_SEPARATOR ) . DIRECTORY_SEPARATOR . 'pdl-csv-imports' . DIRECTORY_SEPARATOR . $state_id;
    ...
    private function setup_working_dir( $csv_file, $images_file = '' ) {
        ...
        $csv_imports_dir = rtrim( $upload_dir['basedir'], DIRECTORY_SEPARATOR ) . DIRECTORY_SEPARATOR . 'pdl-csv-imports';
    ...
    private function set_header( $header ) {
        ...
        $required_fields = pdl_get_form_fields( 'validators=required' );
        ...
        global $pdl;
        $short_names = $pdl->formfields->get_short_names();
    ...
    private function import_row( $data ) {
        ...
        foreach( $categories as &$c ) {
            ...
            if ( $t = term_exists( str_replace( '&', '&amp;', $c['name'] ), PDL_CATEGORY_TAX ) ) {
                $c['term_id'] = $t['term_id'];
            } else {
                $t = wp_insert_term( str_replace( '&amp;', '&', $c['name'] ), PDL_CATEGORY_TAX );
                $a = $t['term_id'];

                if ( is_array( $t ) && isset( $t['term_id'] ) ) {
                    $c['term_id'] = $t['term_id'];
                } else if ( is_wp_error( $t ) ) {
                    $message = _x( 'Could not create listing category "<category-name>". The operation failed with the following error: <error-message>.', 'admin csv-import', 'PDM' );
                    $message = str_replace( '<category-name>', $c['name'], $message );
                    $message = str_replace( '<error-message>', $t->get_error_message(), $message );

                    $errors[] = $message;
                } else {
                    $errors[] = sprintf( _x( 'Could not create listing category "%s"', 'admin csv-import', 'PDM'), $c['name'] );
                }
            }
        ...
        if ( $meta['sequence_id'] ) {
            $listing_id = intval( $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = %s AND meta_value = %s LIMIT 1",
                                                                  '_pdl[import_sequence_id]', $meta['sequence_id'] ) ) );
            if ( PDL_POST_TYPE != get_post_type( $listing_id ) )
                $listing_id = 0;
        }

        // Handle fields.
        foreach ( $fields as $field_id => $field_data ) {
            $f = pdl_get_form_field( $field_id );
        ...
        $listing_data['post_status'] = $listing_id ? pdl_get_option( 'edit-post-status' ) : $this->settings['post-status'];
        ...
        $listing = pdl_save_listing( $listing_data, true, 'csv-import' );
        ...
        if ( $payment && current_time( 'timestamp' ) - strtotime( $payment->created_at ) < 60 ) {
            ...
            pdl_insert_log( array(
                'log_type' => 'payment.note',
                'object_id' => $payment->id,
                'actor' => is_admin() ? 'user:' . get_current_user_id() : 'system',
                'message' => __( 'Listing imported by admin. Payment skipped.', 'PDM' )
            ) );
        }
    ...
    private function sanitize_and_validate_row( $data ) {
        ...
        foreach ( $this->get_header() as $i => $col_info ) {
            ...
            $field = $col_info['field_id'] ? pdl_get_form_field( $col_info['field_id'] ) : null;
            ...
            switch( $column ) {
                ...
                case 'username':
                    if ( $this->settings['assign-listings-to-user'] && $value ) {
                        if ( ! username_exists( $value ) ) {
                            $errors[] = sprintf( _x( 'Username "%s" does not exist', 'admin csv-import', 'PDM' ), $value );
                        } else {
                            $meta['username'] = $value;
                        }
                    }
                ...
                case 'expires_on':
                    ...
                    if ( ! $date ) {
                        $message = _x( "The string <string> couldn't be converted into a valid date.", 'admin csv-import', 'PDM' );
                ...
                case 'fee_id':
                    ...
                    $plan = pdl_get_fee_plan( $submitted_fee_id );

                    if ( ! $plan ) {
                        $message = _x( 'There is no Fee Plan with ID = <fee-id>', 'admin csv-import', 'PDM' );
                ...
                default:
                    ...
                    if ( $field->is_required() && $field->is_empty_value( $value ) ) {
                        $errors[] = sprintf( _x( 'Missing required field: %s', 'admin csv-import', 'PDM' ), $column );
                        break;
                    }
                    ...
                    if ( 'category' == $field->get_association() ) {
                        ...
                        foreach ( $csv_categories as $csv_category_ ) {
                            ...
                            if ( $term = term_exists( $csv_category, PDL_CATEGORY_TAX ) ) {
                                $categories[] = array( 'name' => $csv_category, 'term_id' => $term['term_id'] );
                            } else {
                                if ( ! $this->settings['create-missing-categories'] ) {
                                    $errors[] = sprintf( _x( 'Listing category "%s" does not exist', 'admin csv-import', 'PDM' ), $csv_category );
                                    continue;
                                }
    ...
    private function upload_image( $filename ) {
        ...
        // Make a copy of the file because pdl_media_upload() moves the original file.
        copy( $filepath, $filepath . '.backup' );
        $media_id = pdl_media_upload( $filepath, true, true );
