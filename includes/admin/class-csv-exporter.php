...
class PDL_CSVExporter {
    ...
    public function __construct( $settings, $workingdir=null, $listings=array() ) {
        ...
        if ( !$workingdir ) {
            ...
            if ( !$upload_dir['error'] ) {
                $csvexportsdir = rtrim( $upload_dir['basedir'], DIRECTORY_SEPARATOR ) . DIRECTORY_SEPARATOR . 'pdl-csv-exports';
                if ( is_dir( $csvexportsdir ) || mkdir( $csvexportsdir ) ) {
                    ...
                    if ( !mkdir( $this->workingdir, 0777 ) )
                        $direrror = _x( 'Could not create a temporary directory for handling this CSV export.', 'admin csv-export', 'PDM' );
                } else {
                    $direrror = _x( 'Could not create pdl-csv-exports directory.', 'admin csv-export', 'PDM' );
                }
            }

            if ( $direrror )
                throw new Exception( sprintf( _x( 'Error while creating a temporary directory for CSV export: %s', 'admin csv-export', 'PDM' ), $direrror ) );
        } else {
    ...
    public static function &from_state( $state ) {
        ...
        // Setup columns.
        $shortnames = pdl_formfields_api()->get_short_names();
        ...
        foreach ( $state['columns'] as $fshortname ) {
            ...
            $export->columns[ $fshortname ] = pdl_get_form_field( $field_id );
        }
    ...
    public function cleanup() {
        ...
        pdl_rrmdir( $this->workingdir );

        if ( !$upload_dir['error'] ) {
            $csvexportsdir = rtrim( $upload_dir['basedir'], DIRECTORY_SEPARATOR ) . DIRECTORY_SEPARATOR . 'pdl-csv-exports';
            $contents = pdl_scandir( $csvexportsdir );

            if ( !$contents )
                pdl_rrmdir( $csvexportsdir );
        }
    }
        ...
        foreach ( $this->columns as $column_name => $column_obj ) {
            ...
            default:
                if ( is_object( $column_obj ) ) {
                    ...
                    case 'category':
                    case 'tags':
                        $value = wp_get_post_terms( $listing->get_id(), ( 'tags' == $field->get_association() ? PDL_TAGS_TAX : PDL_CATEGORY_TAX ), 'fields=names' );
