...
require_once( PDL_PATH . 'includes/utils.php' );

class PDL__Migrations__18_0 extends PDL__Migration {
    ...
    public function migrate() {
        ...
        $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}pdl_payments WHERE listing_id NOT IN (SELECT ID FROM {$wpdb->posts} WHERE post_type = %s)", PDL_POST_TYPE ) );
        $wpdb->query( "DELETE FROM {$wpdb->prefix}pdl_payments_items WHERE payment_id NOT IN (SELECT id FROM {$wpdb->prefix}pdl_payments)" );
        $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}pdl_payments SET status = %s WHERE status = %s", 'failed', 'rejected' ) );
    ...
    /**
     * Sets an option that tells PDL that a Featured levels migration is pending.
    ...
    public function _set_featured_migration_flag( &$msg ) {
        update_option( 'pdl-migrate-18_0-featured-pending', true, false );
        return true;
    }
    ...
    public function _migrate_licenses( &$msg ) {
        ...
        $settings = get_option( 'pdl_settings', array() );
        $licenses = get_option( 'pdl_licenses', array() );

        $module_keys = $wpdb->get_results( "SELECT option_name, option_value FROM {$wpdb->options} WHERE option_name LIKE 'pdl-license-key-%'", ARRAY_A );
        $theme_keys  = get_option( 'pdl-themes-licenses', array() );

        foreach ( $module_keys as $item ) {
            $module_id = str_replace( 'pdl-license-key-', '', $item['option_name'] );
            $key       = $item['option_value'];
            $status    = get_option( 'pdl-license-status-' . $module_id, 'invalid' );
        ...
        update_option( 'pdl_settings', $settings );
        update_option( 'pdl_licenses', $licenses );

        $msg = _x( 'Migrating license information to new format...', 'installer', 'PDM' );
        return true;
    }

    public function _migrate_email_notices( &$msg ) {
        require_once( PDL_INC . 'admin/settings/class-settings-bootstrap.php' );
        $defaults = PDL__Settings__Bootstrap::get_default_expiration_notices();

        $notices = array();

        if ( true ) {
            if ( $email = get_option( 'pdl-listing-renewal-message', false ) ) {
                $notices[] = array(
                    'event' => 'expiration',
        ...
        if ( $t = get_option( 'pdl-renewal-email-threshold', false ) ) {
            if ( $email = get_option( 'pdl-renewal-pending-message' ) ) {
                $notices[] = array(
                    'event'         => 'expiration',
            ...
            if ( get_option( 'pdl-send-autorenewal-expiration-notice', false ) ) {
                if ( $email = get_option( 'pdl-listing-autorenewal-notice', false ) ) {
                    $notices[] = array(
                        'event'         => 'expiration',
                        'relative_time' => '+' . $t . ' days',
                        'listings'      => 'recurring',
        ...
        if ( $t = get_option( 'pdl-renewal-reminder-threshold', false ) ) {
            if ( $email = get_option( 'pdl-renewal-reminder-message' ) ) {
                $notices[] = array(
                    'event'         => 'expiration',
                    'relative_time' => '-' . $t . ' days',
                    'listings'      => 'both',
        ...
        if ( true ) {
            if ( $email = get_option( 'pdl-listing-autorenewal-message' ) ) {
                $notices[] = array(
                    'event'         => 'renewal',
                    'listings'      => 'recurring',
                    'subject'       => $email['subject'],
        ...
        update_option( 'pdl-expiration-notices', $notices );

        $msg = _x( 'Migrating email notices to new format...', 'installer', 'PDM' );
        return true;
    }
    ...
    public function _migrate_fee_plans( &$msg ) {
        ...
        $msg = _x( 'Migrating fee plans...', 'installer', 'PDM' );

        if ( ! pdl_table_exists( $wpdb->prefix . 'pdl_fees' ) ) {
            return true;
        }

        foreach ( $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}pdl_fees" ) as $fee ) {
            ...
            $row = array(
                ...
                'recurring' => ( 0 != $fee->days && $fee->amount > 0.0 && get_option( 'pdl-listing-renewal-auto' ) && get_option( 'pdl-listing-renewal-auto-dontask' ) ) ? 1 : 0
            );

            // Check if fee plan already exists.
            $exists  = (bool) $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}pdl_plans WHERE id = %d", $fee->id ) );
            $success = true;

            global $wpdb;
            $wpdb->show_errors();
            if ( $exists ) {
                $success = ( false !== $wpdb->update( $wpdb->prefix . 'pdl_plans', $row, array( 'id' => $fee->id ) ) );
            } else {
                $success = ( false !== $wpdb->insert( $wpdb->prefix . 'pdl_plans', $row ) );
            }

            if ( ! $success ) {
                $msg = sprintf( _x( '! Could not migrate fee "%s" (%d)', 'installer', 'PDM' ), $fee->label, $fee->id );
                return false;
            }
        }
    ...
    public function _migrate_payment_items( &$msg ) {
        ...
        $count = $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}pdl_payments WHERE payment_items IS NULL OR payment_items = %s", '' ) );
        $batch_size = 20;

        if ( ! $count )
            return true;

        foreach ( $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_payments WHERE payment_items IS NULL OR payment_items = %s ORDER BY id ASC LIMIT {$batch_size}", '' )  ) as $payment ) {
            ...
            foreach ( $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_payments_items WHERE payment_id = %d", $payment_id ) ) as $item ) {
                $new_item = array();
            ...
            if ( false === $wpdb->update( $wpdb->prefix . 'pdl_payments', array( 'payment_items' => serialize( $items ), 'payment_type' => $payment_type ), array( 'id' => $payment_id ) ) ) {
                $msg = sprintf( _x( '! Could not migrate payment #%d', 'installer', 'PDM' ), $payment_id );
                return false;
            }
        }

        $msg = sprintf( _x( 'Updating payment items format: %d items remaining...', 'installer', 'PDM' ), max( $count - $batch_size, 0 ) );
        return false;
    }
    ...
    public function _migrate_listings( &$msg ) {
        ...
        if ( ! pdl_table_exists( $wpdb->prefix . 'pdl_listing_fees' ) ) {
            return true;
        }
        ...
        $count = absint( $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->posts} p WHERE p.post_type = %s AND p.ID NOT IN (SELECT lp.listing_id FROM {$wpdb->prefix}pdl_listings lp) ORDER BY ID ASC LIMIT {$batch_size}", PDL_POST_TYPE ) ) );
        $listings = $wpdb->get_col( $wpdb->prepare( "SELECT ID FROM {$wpdb->posts} p WHERE p.post_type = %s AND p.ID NOT IN (SELECT lp.listing_id FROM {$wpdb->prefix}pdl_listings lp) ORDER BY ID ASC LIMIT {$batch_size}", PDL_POST_TYPE ) );
        ...
        foreach ( $listings as $listing_id ) {
            ...
            if ( ! $new_plan ) {
                ...
                $new_plan = array(
                    ...
                    'fee_id' => $free_plan->id,
                    'fee_price' => 0.0,
                    'fee_days' => ! empty( $free_plan->days ) ? $free_plan->days : get_option( 'pdl-listing-duration', 0 ),
                    'fee_images' => ! empty( $free_plan->images ) ? absint( $free_plan->images ) : get_option( 'pdl-free-images', 0 ),
                ...
                $recurring_data = $wpdb->get_var( $wpdb->prepare( "SELECT recurring_data FROM {$wpdb->prefix}pdl_listing_fees WHERE recurring_data IS NOT NULL AND listing_id = %d LIMIT 1", $listing_id ) );
                if ( ! empty( $new_plan['is_recurring'] ) && $new_plan['is_recurring'] && $recurring_data )
                    $new_plan['recurring_data'] = $recurring_data;
            }

            $wpdb->delete( $wpdb->prefix . 'pdl_listings', array( 'listing_id' => $listing_id ) );
            $wpdb->insert( $wpdb->prefix . 'pdl_listings', $new_plan );

            $l = PDL_Listing::get( $listing_id );
            $l->get_status( true );
        }

        $msg = sprintf( _x( 'Migrating listing information: %d items remaining...', 'installer', 'PDM' ), max( $count - $batch_size, 0 ) );
        return false;
    }
    ...
    private function set_listing_categories( $listing_id ) {
        ...
        // From current fees.
        $cat_ids = array_merge( $cat_ids, $wpdb->get_col( $wpdb->prepare( "SELECT category_id FROM {$wpdb->prefix}pdl_listing_fees WHERE listing_id = %d", $listing_id ) ) );

        // From pending payments.
        $pending = $wpdb->get_col( $wpdb->prepare( "SELECT payment_items FROM {$wpdb->prefix}pdl_payments WHERE listing_id = %d AND status = %s", $listing_id, 'pending' ) );
        if ( $pending ) {
        ...
        if ( $cat_ids )
            wp_set_object_terms( $listing_id, $cat_ids, PDL_CATEGORY_TAX, true );
    }
    ...
    private function plan_from_fees( $listing_id ) {
        ...
        $fees = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_listing_fees WHERE listing_id = %d", $listing_id ) );
    ...
    private function plan_from_payments( $listing_id ) {
        ...
        $fee = null;
        $pending_payments = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_payments WHERE listing_id = %d AND status = %s", $listing_id, 'pending' ) );
