...
class PDL__Migrations__3_2 extends PDL__Migration {

    /*
     * This update converts all form fields to a new, more flexible format that uses a new API introduced in PDL 2.3.
     */
    public function migrate() {
        ...
        $old_fields = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}pdl_form_fields" );
        ...
        foreach ( $old_fields as &$f ) {
            ...
            $wpdb->update( "{$wpdb->prefix}pdl_form_fields", $newfield, array( 'id' => $f->id ) );
        }

        $wpdb->query( "ALTER TABLE {$wpdb->prefix}pdl_form_fields DROP COLUMN validator;" );
        $wpdb->query( "ALTER TABLE {$wpdb->prefix}pdl_form_fields DROP COLUMN display_options;" );
        $wpdb->query( "ALTER TABLE {$wpdb->prefix}pdl_form_fields DROP COLUMN is_required;" );
        $wpdb->query( "ALTER TABLE {$wpdb->prefix}pdl_form_fields DROP COLUMN type;" );
    ...
    public function disable_regions_in_3_2_upgrade() {
        ...
        if ( class_exists( 'PDL_RegionsPlugin' ) && version_compare( PDL_RegionsPlugin::VERSION, '1.1', '<' ) ) {
            deactivate_plugins( 'directory-listing-regions/directory-listing-regions.php', true );
            echo sprintf( '<div class="error"><p>%s</p></div>',
                          _x( '<b>Plestar Directory Listing - Regions Module</b> was disabled because it is incompatible with the current version of Plestar Directory Listing. Please update the Regions module.', 'installer', 'PDM' )
                        );
        }        
    }
