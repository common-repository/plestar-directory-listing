...
class PDL__Migrations__5_0 extends PDL__Migration {
    ...
    private function gather_pre_split_term_ids() {
        ...
        // Fees.
        $fees = $wpdb->get_col( "SELECT categories FROM {$wpdb->prefix}pdl_fees" );
        foreach ( $fees as $f ) {
            $data = unserialize( $f );
        ...
        // Listing fees.
        if ( $fee_ids = $wpdb->get_col( "SELECT DISTINCT category_id FROM {$wpdb->prefix}pdl_listing_fees" ) ) {
            $res = array_merge( $res, $fee_ids );
        }

        // Payments.
        $payments_terms = $wpdb->get_col(
                $wpdb->prepare( "SELECT DISTINCT rel_id_1 FROM {$wpdb->prefix}pdl_payments_items WHERE ( item_type = %s OR item_type = %s )",
                                'fee',
                                'recurring_fee' )
        );
        ...
        // Category images.
        $imgs = get_option( 'pdl[category_images]', false );
    ...
    /**
     * Use this function to update PDL references of a pre-split term ID to use the new term ID.
     * @since 3.6.4
     */
    public function process_term_split( $old_id = 0 ) {
        ...
        $new_id = wp_get_split_term( $old_id, PDL_CATEGORY_TAX );
        if ( ! $new_id )
            return;

        // Fees.
        $fees = $wpdb->get_results( "SELECT id, categories FROM {$wpdb->prefix}pdl_fees" );
        foreach ( $fees as &$f ) {
            ...
            $wpdb->update( $wpdb->prefix . 'pdl_fees',
                           array( 'categories' => serialize( $categories ) ),
                           array( 'id' => $f->id ) );
        }

        // Listing fees.
        $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}pdl_listing_fees SET category_id = %d WHERE category_id = %d",
                                      $new_id,
                                      $old_id ) );

        // Payments.
        $wpdb->query(
            $wpdb->prepare( "UPDATE {$wpdb->prefix}pdl_payments_items SET rel_id_1 = %d WHERE ( rel_id_1 = %d AND ( item_type = %s OR item_type = %s ) )",
                            $new_id,
                            $old_id,
                            'fee',
                            'recurring_fee' )
        ...
        update_option( 'pdl[category_images]', $imgs );
    }
