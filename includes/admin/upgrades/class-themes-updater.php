...
class PDL_Themes_Updater {
    ...
    public function __construct( &$themes_api ) {
        ...
        $this->data = get_transient( 'pdl-themes-updates' );

        if ( ! is_array( $this->data ) )
            $this->check_for_updates();

        add_action( 'wp_ajax_pdl-themes-update', array( &$this, '_update_theme' ) );
        add_action( 'pdl-admin-themes-item-css', array( &$this, '_add_update_css' ) );
        add_action( 'pdl-admin-themes-extra', array( &$this, '_show_update_info' ) );
    }
    ...
    private function check_for_updates() {
        $data = get_transient( 'pdl-themes-updates' );
        if ( ! is_array( $data ) )
            $data = array();
        ...
        set_transient( 'pdl-themes-updates', $data, 60 * 60 * 24 * 2 ); // Make this available for 48 hours.
        $this->data = $data;
    }
    ...
    private function get_latest_version( $theme ) {
        ...
        $response = wp_remote_get( add_query_arg( $request, 'http://plestar.net/' ), array( 'timeout' => 15, 'sslverify' => false ) );
    ...
    public function _show_update_info( $theme ) {
        ...
        printf( '<div class="pdl-theme-update-info update-available" data-l10n-updating="%s" data-l10n-updated="%s">',
                _x( 'Updating theme...', 'themes', 'PDM' ),
                _x( 'Theme updated.', 'themes', 'PDM' ) );
        echo '<div class="update-message">';
        $msg = _x( 'New version available (<b>%s</b>). <a>Update now.</a>', 'themes', 'PDM' );
    ...
    public function _update_theme() {
        ...
        if ( is_wp_error( $result ) )
            $response->send_error( sprintf( _x( 'Could not update theme: %s', 'themes', 'PDM' ), $result->get_error_message() ) );

        $this->themes_api->find_themes( true );

        $response->add( 'html', pdl_render_page( PDL_PATH . 'templates/admin/themes-item.tpl.php',
                                                   array( 'theme' => $this->themes_api->get_theme( $theme_id ) ) ) );
        $response->set_message( _x( 'Theme was updated successfully.', 'themes', 'PDM' ) );
        $response->send();
    }
    ...
    private function run_update( $theme_id ) {
        ...
        if ( ! is_dir( $upgrade_folder ) ) {
            if ( ! PDL_FS::mkdir( $upgrade_folder ) )
