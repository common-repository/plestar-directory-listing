...
class PDL__Meta {
    ...
    public function _rss_feed() {
        ...
        $main_page_title = get_the_title( pdl_get_page_id() );
        ...
        if ( 'main' === $current_view || 'all_listings' === $current_view ) {
            $feed_title = sprintf( _x( '%s Feed', 'rss feed', 'PDM'), $main_page_title );
            $feed_url = esc_url( add_query_arg( 'post_type', PDL_POST_TYPE,  get_bloginfo( 'rss2_url' ) ) );
        ...
        if ( 'show_category' === $current_view ) {
            ...
            if ( $term ) {
                ...
                $query_args = array( 'post_type' => PDL_POST_TYPE, PDL_CATEGORY_TAX => $term->slug );
        ...
        if ( $feed_links ) {
            echo '<!-- Plestar Directory Listing RSS feed -->' . PHP_EOL;
    ...
    public function _meta_setup() {
        ...
        require_once( PDL_PATH . 'includes/class-page-meta.php' );
        $this->page_meta = new PDL_Page_Meta( $action );
        ...
        if ( 'show_listing' == $action && pdl_rewrite_on() ) {
    ...
    public function set_view_title( $title ) {
        ...
        if ( empty( $wp_query->pdl_view ) || ! is_array( $title ) )
            return $title;

        $current_view = pdl()->dispatcher->current_view_object();
    ...
    public function _meta_title( $title = '', $sep = '»', $seplocation = 'right' ) {
        ...
        switch ( $current_view ) {
            case 'submit_listing':
                $view_title =  _x( 'Submit A Listing', 'views', 'PDM' );
                return $this->_maybe_do_wpseo_title( $view_title, $title, $sep, $seplocation );
                break;

            case 'search':
                $view_title =  _x( 'Find a Listing', 'title', 'PDM' );
                return $this->_maybe_do_wpseo_title( $view_title, $title, $sep, $seplocation );
                break;

            case 'all_listings':
                $view_title = _x( 'View All Listings', 'title', 'PDM' );
                return $this->_maybe_do_wpseo_title( $view_title, $title, $sep, $seplocation );
                break;
            ...
            case 'show_tag':
                $term = get_term_by(
                    'slug',
                    get_query_var( '_' . pdl_get_option( 'permalinks-tags-slug' ) ),
                    PDL_TAGS_TAX
                );
                ...
                return sprintf( _x( 'Listings tagged: %s', 'title', 'PDM' ), $term->name ) . ' ' . $sep . ' ' . $title;
            ...
            case 'show_category':
                $term = get_term_by(
                    'slug',
                    get_query_var( '_' . pdl_get_option( 'permalinks-category-slug' ) ),
                    PDL_CATEGORY_TAX
                );

                if ( ! $term && get_query_var( 'category_id' ) ) {
                    $term = get_term_by( 'id', get_query_var( 'category_id' ), PDL_CATEGORY_TAX );
                }
            ...
            case 'show_listing':
                $listing_id = pdl_get_post_by_id_or_slug(
                    get_query_var( '_' . pdl_get_option( 'permalinks-directory-slug' ) ),
                    'id',
                    'id'
                );
    ...
    function listing_opentags() {
        ...
        echo '<meta property="og:title" content="' . esc_attr( PDL_SEO::listing_title( $listing_id ) ) . '" />';
        echo '<meta property="og:url" content="' . esc_url( user_trailingslashit( $listing->get_permalink() ) ) . '" />';
        echo '<meta property="og:description" content="' . esc_attr( PDL_SEO::listing_og_description( $listing_id ) ) . '" />';

        if ( $thumbnail_id = $listing->get_thumbnail_id() ) {
            if ( $img = wp_get_attachment_image_src( $thumbnail_id, 'pdl-large' ) )
                echo '<meta property="og:image" content="' . $img[0] . '" />';
        } else {
            $image_url = PDL_URL . 'assets/images/default-image-big.gif';
