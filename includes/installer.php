<?php
require_once ( PDL_PATH . 'includes/admin/upgrades/class-migration.php' );
...
class PDL_Installer {
    ...
    public function __construct() {
        $this->installed_version = get_option( 'pdl-db-version', get_option( 'wpdirlist_db_version', null ) );
    ...
    public function install() {
        ...
        // TODO: Is this event now unnecessary? Check pdl_daily_events event.
        // schedule expiration hook if needed
        if (!wp_next_scheduled('pdl_listings_expiration_check')) {
            pdl_log('Expiration check was not in schedule. Scheduling.');
            wp_schedule_event(current_time('timestamp'), 'hourly', 'pdl_listings_expiration_check');
        } else {
            // error_log( 'Expiration check was in schedule. Nothing to do.' );
            pdl_log('Expiration check was in schedule. Nothing to do.');
        }

        if ( false === get_option( 'pdl-db-migrations', false ) )
            update_option( 'pdl-db-migrations', array(), false );
        ...
        if ( $this->installed_version ) {
            pdl_log('PDL is already installed.');
            return $this->_update();
        } else if ( $this->_table_exists( "{$wpdb->prefix}pdl_form_fields" ) ) {
            pdl_log('New installation. Creating default form fields.');
            global $pdl;

            // Create default category.
            wp_insert_term( _x( 'General', 'default category name', 'PDM' ), PDL_CATEGORY_TAX );

            $pdl->formfields->create_default_fields();
                        
            add_option( 'pdl-show-tracking-pointer', 1 );

            // Create default paid fee.
            $fee = new PDL__Fee_Plan( array( 'label' => _x( 'Default Fee', 'installer', 'PDM' ),
                                               'amount' => 1.0,
                                               'days' => 365,
                                               'images' => 1,
            ...
            throw new Exception( "Table {$wpdb->prefix}pdl_form_fields was not created!" );
        }

        update_option('pdl-db-version', self::DB_VERSION);
    }

    /**
     * Builds the SQL queries (without running them) used to create all of the required database tables for PDL.
     * Calls the `pdl_database_schema` filter that allows plugins to modify the schema.
    ...
    public function get_database_schema() {
        ...
        $schema['form_fields'] = "CREATE TABLE {$wpdb->prefix}pdl_form_fields (
        ...
        $schema['plans'] = "CREATE TABLE {$wpdb->prefix}pdl_plans (
            id bigint(20) PRIMARY KEY  AUTO_INCREMENT,
            label varchar(255) CHARACTER SET utf8 COLLATE utf8_general_ci NOT NULL,
        ...
        $schema['payments'] = "CREATE TABLE {$wpdb->prefix}pdl_payments (
            id bigint(20) PRIMARY KEY  AUTO_INCREMENT,
        ...
        $schema['listings'] = "CREATE TABLE {$wpdb->prefix}pdl_listings (
        ...
        $schema['logs'] = "CREATE TABLE {$wpdb->prefix}pdl_logs (
            id bigint(20) PRIMARY KEY  AUTO_INCREMENT,
        ...
        return apply_filters( 'pdl_database_schema', $schema );
    }

    public function update_database_schema() {
        require_once(ABSPATH . 'wp-admin/includes/upgrade.php');

        pdl_log( 'Running dbDelta.' );
    ...
    public function _update() {
        ...
        update_option( 'pdl-db-version', $this->installed_version );

        if ( get_option( 'pdl-manual-upgrade-pending', array() ) ) {
            return;
        }
        ...
        foreach ( $migrations as $version ) {
            pdl_log( sprintf( 'Running upgrade routine for version %s', $version ) );

            try {
                $m = $this->load_migration_class( 'PDL__Migrations__' . str_replace( '.', '_', $version ) );
            } catch ( Exception $e ) {
                $m = false;
            }
    ...
    public function update_installed_version( $new_version ) {
        ...
        update_option( 'pdl-db-version', $new_version );
    }

    public function load_migration_class( $classname ) {
        $file = PDL_PATH . 'includes/admin/upgrades/migrations/migration-' . str_replace( 'PDL__Migrations__', '', $classname ) . '.php';
    ...
    public function get_manual_upgrades() {
        $manual_upgrades = get_option( 'pdl-manual-upgrade-pending', array() );
        ...
        } elseif ( isset( $manual_upgrades[0] ) && pdl_starts_with( 'PDL__Migrations__', $manual_upgrades[0] ) ) {
    ...
    public function show_installation_error( $exception ) {
        require_once ( PDL_PATH . 'includes/admin/upgrades/class-installer-installation-error.php' );
        new PDL__Installer__Installation_Error( $exception );
    }
    ...
    public function get_pending_migrations() {
        ...
        foreach ( PDL_FS::ls( PDL_PATH . 'includes/admin/upgrades/migrations/' ) as $_ ) {
            $_ = strtolower( $_ );
            if ( ! pdl_starts_with( basename( $_ ), 'migration-' ) ) {
                continue;
            }
    ...
    public function setup_manual_upgrade() {
        ...
        require_once( PDL_PATH . 'includes/admin/upgrades/class-manual-upgrade-helper.php' );

        try {
            return new PDL__Manual_Upgrade_Helper( $this );
        } catch ( Exception $e ) {
    ...
    public function handle_term_split( $old_id, $new_id, $tt_id, $tax ) {
        if ( PDL_CATEGORY_TAX != $tax )
            return;

        require_once ( PDL_PATH . 'includes/admin/upgrades/migrations/migration-5_0.php' );
        $m = new PDL__Migrations__5_0( $this );
