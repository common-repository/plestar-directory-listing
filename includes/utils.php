<?php
require_once( PDL_INC . 'debugging.php' );
require_once( PDL_INC . 'helpers/class-database-helper.php' );
require_once( PDL_INC . 'helpers/class-email.php' );
require_once( PDL_INC . 'compatibility/class-ajax-response.php' );
require_once( PDL_INC . 'helpers/class-fs.php' );
...
/**
 * @deprecated since next-release. Use {@link PDL__Utils} instead.
 */
class PDL_Utils extends PDL__Utils {}
 ...
 * @return array
 */
function pdl_flatten_files_array( $files = array() ) {
 ...
 *
 */
function pdl_getv($dict, $key, $default=false) {
...
function pdl_capture_action($hook) {
    $output = '';
...
function pdl_capture_action_array($hook, $args=array()) {
    $output = '';
...
function pdl_php_ini_size_to_bytes( $val ) {
    $val = trim( $val );
    $size = intval( $val );
...
function pdl_media_upload_check_env( &$error ) {
    if ( empty( $_FILES ) && empty( $_POST ) && isset( $_SERVER['REQUEST_METHOD'] ) &&
         strtolower( $_SERVER['REQUEST_METHOD'] ) == 'post' ) {
        $post_max = pdl_php_ini_size_to_bytes( ini_get( 'post_max_size' ) );
        $posted_size = intval( $_SERVER['CONTENT_LENGTH'] );

        if ( $posted_size > $post_max ) {
            $error = _x( 'POSTed data exceeds PHP config. maximum. See "post_max_size" directive.', 'utils', 'PDM' );
            return false;
        }
    }
 ...
 * @since 2.1.6
 */
function pdl_media_upload($file_, $use_media_library=true, $check_image=false, $constraints=array(), &$error_msg=null, $sideload=false) {
    ...
    if ( $sideload ) {
        $mime_type = pdl_get_mimetype( $file_ );
    ...
    if ($file['error'] == 0) {
        if ($constraints['max-size'] > 0 && $file['size'] > $constraints['max-size'] ) {
            $error_msg = sprintf( _x( 'File size (%s) exceeds maximum file size of %s', 'utils', 'PDM' ),
        ...
        if ( $constraints['min-size'] > 0 && $file['size'] < $constraints['min-size'] ) {
            $error_msg = sprintf( _x( 'File size (%s) is inferior to the required minimum file size of %s', 'utils', 'PDM' ),
                                size_format( $file['size'], 2 ),
        ...
        if ( is_array( $constraints['mimetypes'] ) ) {
            if ( !in_array( strtolower( $file['type'] ), $constraints['mimetypes'] ) ) {
                $error_msg = sprintf( _x( 'File type "%s" is not allowed', 'utils', 'PDM' ), $file['type'] );
                return false;
            }
        }
        ...
        if( ! $upload || ! is_array( $upload ) || isset( $upload['error'] ) ) {
            $error_msg = isset( $upload['error'] ) ? $upload['error'] : _x( 'Unkown error while uploading file.', 'utils', 'PDM' );
            return false;
        }
        ...
        if ( $attachment_id = wp_insert_attachment(array(
            ...
            if ( $check_image && ! wp_attachment_is_image( $attachment_id ) ) {
                ...
                $error_msg = _x('Uploaded file is not an image', 'utils', 'PDM');
                return false;
            }
            ...
            if ( wp_attachment_is_image( $attachment_id ) ) {
                ...
                if ( ! $failed && $meta && $constraints['min-width'] > 0 && $meta['width'] < $constraints['min-width'] ) {
                    $error_msg = sprintf( _x( 'Image width (%s px) is inferior to minimum required width of %s px.', 'utils', 'PDM' ),
                                          $meta['width'],
                                          $constraints['min-width'] );
                }

                if ( ! $failed && $meta && $constraints['min-height'] > 0 && $meta['height'] < $constraints['min-height'] ) {
                    $error_msg = sprintf( _x( 'Image height (%s px) is inferior to minimum required height of %s px.', 'utils', 'PDM' ),
                                          $meta['height'],
                                          $constraints['min-height'] );
                }

                if ( ! $failed && $meta && $constraints['max-width'] > 0 && $meta['width'] > $constraints['max-width'] ) {
                    $error_msg = sprintf( _x( 'Image width (%s px) is greater than maximum allowed width of %s px.', 'utils', 'PDM' ),
                                          $meta['width'],
                                          $constraints['max-width'] );
                }

                if ( ! $failed && $meta && $constraints['max-height'] > 0 && $meta['height'] > $constraints['max-height'] ) {
                    $error_msg = sprintf( _x( 'Image height (%s px) is greater than maximum required height of %s px.', 'utils', 'PDM' ),
                                          $meta['height'],
        ...
        $error_msg = _x('Error while uploading file', 'utils', 'PDM');
    }
 ...
 * @since 5.0.5
 */
function pdl_get_mimetype( $file ) {
    $mime_type = null;
 ...
 */
function pdl_get_current_domain($www=true, $prefix='') {
    $domain = pdl_getv($_SERVER, 'HTTP_HOST', '');
    if (empty($domain)) {
        $domain = pdl_getv($_SERVER, 'SERVER_NAME', '');
    }
 ...
 * @since 2.1.5
 */
function pdl_ajaxurl($overwrite=false) {
 ...
 * @since 2.3
 */
function pdl_array_remove_value( &$array_, &$value_ ) {
 ...
 * @since 3.0.3
 */
function pdl_starts_with( $str, $prefix, $case_sensitive=true ) {
    if ( !$case_sensitive )
 ...
 * @since 3.1
 */
function pdl_format_time( $time=null, $format='mysql', $time_is_date=false ) {
 ...
 * @since 3.3
 */
function pdl_scandir( $path, $args = array() ) {
    if ( !is_dir( $path ) )
        return array();
/*
    $defaults = array(
 ...
 * @param string $path a directory.
 * @since 3.3
 * @deprecated since 3.6.10. Use {@link PDL_FS::rmdir} instead.
 */
function pdl_rrmdir( $path ) {
    return PDL_FS::rmdir( $path );
}

/**
 * Returns the name of a term.
 * @param id|string $id_or_slug The term ID or slug (see `$field`).
 * @param string $taxonomy Taxonomy name. Defaults to `PDL_CATEGORY_TAX` (PDL's category taxonomy).
 ...
 * @since 3.3
 */
function pdl_get_term_name( $id_or_slug, $taxonomy = PDL_CATEGORY_TAX, $field = 'id', $escape = true ) {
...
function pdl_has_shortcode( &$content, $shortcode ) {
 ...
 * @since 3.4.2
 */
function pdl_text_from_template( $setting_name, $replacements = array() ) {
    $setting = pdl()->settings->get_setting( $setting_name );

    if ( ! $setting )
        return false;

    $text = pdl_get_option( $setting_name );
 ...
 * @since 3.5.4
 */
function pdl_email_from_template( $setting_or_file, $replacements = array(), $args = array() ) {
    $setting = null;
    $file = null;
    $object = null;
    ...
    if ( is_string( $setting_or_file ) && is_file( $setting_or_file ) && is_readable( $setting_or_file ) ) {
        ...
        $setting = pdl()->settings->get_setting( $setting_or_file );
    }

    if ( ( ! $setting && ! $file && ! $object ) || ( $setting && 'email_template' != $setting['type'] ) )
        return false;

    if ( ! class_exists( 'PDL_Email' ) )
        require_once( PDL_PATH . 'includes/helpers/class-email.php' );
    ...
    $replacements = array_merge( $replacements, array(
        ...
        'directory-url' => sprintf( '<a href="%1$s">%1$s</a>', pdl_get_page_link( 'main' ) ),
    ...
    if ( $setting || $object ) {
        $value = $setting ? pdl_get_option( $setting['id'] ) : (array) $object;
        ...
        $body = pdl_render_page( $file, $replacements );
    }
...
function pdl_admin_pointer( $selector, $title, $content_ = '',
...
//<![CDATA[
jQuery(function( $ ) {
        var pdl_pointer = $( '<?php echo $selector; ?>' ).pointer({
            ...
            'buttons': function( e, t ) {
                ...
                var b = $( '<a id="pdl-pointer-b1" class="button button-primary">' + '<?php echo $primary_button; ?>' + '</a>' );
                <?php else: ?>
                var b = $( '<a id="pdl-pointer-b2" class="button" style="margin-right: 15px;">' + '<?php echo $secondary_button; ?>' + '</a>' );
                <?php endif; ?>
                return b;
            }
        }).pointer('open');
        ...
        <?php if ( $secondary_button ): ?>
        $( '#pdl-pointer-b2' ).before( '<a id="pdl-pointer-b1" class="button button-primary">' + '<?php echo $primary_button; ?>' + '</a>' );
        $( '#pdl-pointer-b2' ).click(function(e) {
            e.preventDefault();
            <?php if ( $secondary_action ): ?>
            <?php echo $secondary_action; ?>
            <?php endif; ?>
            pdl_pointer.pointer( 'close' );
        });
        <?php endif; ?>

        $( '#pdl-pointer-b1' ).click(function(e) {
            e.preventDefault();
 ...
 * @since 4.0.5dev
 */
function pdl_detect_encoding( $content ) {
    ...
    if ( function_exists( 'mb_detect_encoding' ) ) {
        ...
        if ( ! function_exists( 'iconv' ) )
            ...
            return pdl_mb_detect_encoding( $content, $encodings );
    }
}
 ...
 * @since 4.0.5dev
 */
function pdl_mb_detect_encoding( $content, $encodings ) {
...
function pdl_render_user_field( $args = array() ) {
    ...
    if ( $users_query->get_total() <= 200 ) {
        ...
        $output = '<input class="pdl-user-autocomplete ' . esc_attr( $args['class'] ) . '" type="text" value="' . esc_attr( $text_value ) . '" data-hidden-field="' . $hidden_field_id . '" />';
...
function pdl_enqueue_jquery_ui_style() {
    global $wp_scripts;
    ...
    wp_enqueue_style('pdl-jquery-ui-css',
    PDL_URL.'assets/css/jquery-ui.css',
    false,
    PLUGIN_VERSION,
    false);
}

function pdl_buckwalter_arabic_transliteration( $content ) {
 ...
 * PDL_Form_Field_Type. It has always rendered values as given, indirectly
 ...
 * @since 4.1.10
 */
function pdl_html_attributes( $attrs, $exceptions = array() ) {
    $html = '';
 ...
 * @since 4.1.11
 */
function pdl_table_exists( $table_name ) {
    global $wpdb;
 ...
 * @since 5.0.5
 */
function pdl_column_exists( $table_name, $column_name ) {
    global $wpdb;
 ...
 * @since 5.0
 */
function pdl_deprecation_warning( $msg = '' ) {
    global $pdl_deprecation_warnings;


    if ( ! isset( $pdl_deprecation_warnings ) ) {
        $pdl_deprecation_warnings = array();
    }

    $pdl_deprecation_warnings[] = $msg;
}
