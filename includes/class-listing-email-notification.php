 ...
 * @since 5.0
 */
class PDL__Listing_Email_Notification {

    public function __construct() {
        add_action( 'transition_post_status', array( $this, 'listing_published_notification' ), 10, 3 );
        add_action( 'pdl_listing_status_change', array( $this, 'status_change_notifications' ), 10, 3 );
        add_action( 'pdl_edit_listing', array( $this, 'edit_listing_admin_email' ) );

        add_action( 'pdl_listing_renewed', array( $this, 'listing_renewal_email' ), 10, 3 );

        add_action( 'pdl_listing_maybe_send_notices', array( $this, 'send_notices' ), 10, 3 );

        add_action( 'pdl_listing_maybe_flagging_notice', array( $this, 'reported_listing_email' ), 10, 2 );
    }
    ...
    public function listing_published_notification( $new_status, $old_status, $post ) {
        if ( ! in_array( 'listing-published', pdl_get_option( 'user-notifications' ), true ) )
            return;
        ...
        if ( isset( $pdl->_importing_csv_no_email ) && $pdl->_importing_csv_no_email )
            return;
    ...
    public function send_listing_published_notification( $post_id, $post ) {
        if ( ! isset( $post->post_type ) || PDL_POST_TYPE !== $post->post_type ) {
            return;
        }
        $email = pdl_email_from_template( 'email-templates-listing-published', array(
            'listing' => get_the_title( $post->ID ),
            'listing-url' => get_permalink( $post->ID )
        ) );

        $email->to[]     = wpdirlist_get_the_business_email( $post->ID );
    ...
    public function status_change_notifications( $listing, $old_status, $new_status ) {
        // Expiration notice.
        if ( 'expired' == $new_status && pdl_get_option( 'listing-renewal' ) ) {
    ...
    public function send_notices( $event, $relative_time, $listing, $force_resend = false ) {
        $listing = is_object( $listing ) ? $listing : pdl_get_listing( absint( $listing ) );
        if ( ! $listing ) {
            return;
        }
        ...
        $all_notices = pdl_get_option( 'expiration-notices' );
        ...
        foreach ( $all_notices as $notice_key => $notice ) {
            ...
            $already_sent = (int) get_post_meta( $listing->get_id(), '_pdl_notice_sent_' . $notice_key, true );
            ...
            $email = pdl_email_from_template(
                ...
                array(
                    ...
                    'category'      => get_the_term_list( $listing->get_id(), PDL_CATEGORY_TAX, '', ', ' ),
            ...
            if ( 'expiration' == $event && in_array( 'renewal', pdl_get_option( 'admin-notifications' ), true ) ) {
                $email->cc[] = get_option( 'admin_email' );

                if ( pdl_get_option( 'admin-notifications-cc' ) )
                    $email->cc[] = pdl_get_option( 'admin-notifications-cc' );
            }

            if ( $email->send() ) {
                // update_post_meta( $listing->get_id(), '_pdl_notice_sent_' . $notice_key, current_time( 'timestamp' ) );
            }
        }
    }
    ...
    private function send_new_listing_email( $listing ) {
        ...
        // Notify the admin.
        if ( in_array( 'new-listing', pdl_get_option( 'admin-notifications' ), true ) ) {
            $admin_email = new PDL_Email();
            $admin_email->subject = sprintf( _x( '[%s] New listing notification', 'notify email', 'PDM' ), get_bloginfo( 'name' ) );
            $admin_email->to[] = get_bloginfo( 'admin_email' );

            if ( pdl_get_option( 'admin-notifications-cc' ) )
                $admin_email->cc[] = pdl_get_option( 'admin-notifications-cc' );

            $admin_email->body = pdl_render( 'email/listing-added', array( 'listing' => $listing ), false );
            $admin_email->send();
        }

        // Notify the submitter.
        if ( in_array( 'new-listing', pdl_get_option( 'user-notifications' ), true ) ) {
            $email = pdl_email_from_template( 'email-confirmation-message', array(
    ...
    public function edit_listing_admin_email( $listing_id ) {
        ...
        if ( ! in_array( 'listing-edit', pdl_get_option( 'admin-notifications' ), true ) )
            return;

        $listing = pdl_get_listing( $listing_id );

        $email = new PDL_Email();
        $email->subject = sprintf( _x( '[%s] Listing edit notification', 'notify email', 'PDM' ), get_bloginfo( 'name' ) );
        $email->to[] = get_bloginfo( 'admin_email' );

        if ( pdl_get_option( 'admin-notifications-cc' ) )
            $email->cc[] = pdl_get_option( 'admin-notifications-cc' );

        $email->body = pdl_render( 'email/listing-edited', array( 'listing' => $listing ), false );
    ...
    public function listing_renewal_email( $listing, $payment = false, $context = '' ) {
        // Notify admin.
        if ( in_array( 'after_renewal', pdl_get_option( 'admin-notifications' ), true ) ) {
            $email = new PDL_Email();
            $email->to[] = get_bloginfo( 'admin_email' );
            $email->subject = sprintf( '[%s] Listing "%s" has renewed', get_bloginfo( 'name' ), $listing->get_title() );

            if ( $cc = pdl_get_option( 'admin-notifications-cc' ) ) {
                $email->cc[] = $cc;
            }

            $owner = wpdirlist_get_the_business_email( $listing->get_id() );
            if ( ! empty( $payment ) ) {
            ...
            $amount = pdl_currency_format( $amount );
        ...
        // Notify users.
        do_action( 'pdl_listing_maybe_send_notices', 'renewal', '0 days', $listing );
    }

    public function reported_listing_email( $listing, $report ) {
        // Notify the admin.
        if ( in_array( 'flagging_listing', pdl_get_option( 'admin-notifications' ), true ) ) {
            $admin_email = new PDL_Email();
            $admin_email->subject = sprintf( _x( '[%s] Reported listing notification', 'notify email', 'PDM' ), get_bloginfo( 'name' ) );
            $admin_email->to[] = get_bloginfo( 'admin_email' );

            if ( pdl_get_option( 'admin-notifications-cc' ) )
                $admin_email->cc[] = pdl_get_option( 'admin-notifications-cc' );

            $admin_email->body = pdl_render( 'email/listing-reported', array( 'listing' => $listing, 'report' => $report ), false );
            $admin_email->send();
        }
    }
