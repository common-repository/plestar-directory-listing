...
class PDL_Payment extends PDL__DB__Model {
    ...
    protected function get_defaults() {
        return array(
            ...
            'gateway_data' => array(),
            'status' => 'pending',
            'currency_code' => pdl_get_option( 'currency', 'USD' ),
            'amount' => 0.0,
            'data' => array()
        );
    }
    ...
    protected function prepare_row() {
        ...
        // FIXME: In the future we should not use PDL__DB__Model at all. See #2945.
    ...
    protected function before_save( $new = false ) {
        if ( ! $this->payment_key )
            $this->payment_key = strtolower( sha1( $this->listing_id . date( 'Y-m-d H:i:s' ) . ( defined( 'AUTH_KEY' ) ? AUTH_KEY : '' ) . uniqid( 'pdl', true ) ) );
    ...
    protected function after_save( $new = false ) {
        ...
        if ( $this->old_status != $this->status ) {
            pdl_insert_log( array(
                ...
                'object_id' => $this->id,
                'message' => sprintf( _x( 'Payment status changed from "%s" to "%s".', 'payment', 'PDM' ), $this->old_status, $this->status ) ) );
            do_action_ref_array( 'PDL_Payment::status_change', array( &$this, $this->old_status, $this->status ) );
            do_action( "pdl_payment_{$this->status}", $this );
        }
    ...
    protected function after_delete() {
        global $wpdb;
        $wpdb->delete( $wpdb->prefix . 'pdl_logs', array( 'object_type' => 'payment', 'object_id' => $this->id ) );
    }
    ...
    public function get_listing() {
        return PDL_Listing::get( $this->listing_id );
    }
    ...
    public function get_summary() {
        ...
        case 'initial':
            $summary = sprintf( _x( 'Initial payment ("%s")', 'payment', 'PDM' ), $this->get_listing()->get_title() );
            break;
        case 'renewal':
            $summary = sprintf( _x( 'Renewal payment ("%s")', 'payment', 'PDM' ), $this->get_listing()->get_title() );
            break;
        default:
            break;
        }
        ...
        if ( 'admin-submit' == $this->context ) {
            $summary = sprintf( _x( '%s. Admin Posted.', 'payment summary', 'PDM' ), $summary );
        } else if ( 'csv-import' == $this->context ) {
            $summary = sprintf( _x( '%s. Imported Listing.', 'payment summary', 'PDM' ), $summary );
        }
    ...
    public function process_as_admin() {
        // $this->payment_items[0]['description'] .= ' ' . _x( '(admin, no charge)', 'submit listing', 'PDM' );
        ...
        pdl_insert_log( array(
            ...
            'object_id' => $this->id,
            'actor' => is_admin() ? 'user:' . get_current_user_id() : 'system',
            'message' => _x( 'Listing submitted by admin. Payment skipped.', 'submit listing', 'PDM' ) ) );
    }
    ...
    public function get_admin_url() {
        return admin_url( 'admin.php?page=pdl_admin_payments&pdl-view=details&payment-id=' . $this->id );
    }

    public function get_checkout_url( $force_http = false ) {
        $url = pdl_url( 'checkout', $this->payment_key );

        if ( ! $force_http && ! is_ssl() && pdl_get_option( 'payments-use-https' ) )
    ...
    public function get_return_url() {
        $params = array(
            'action' => 'return',
            '_wpnonce' => wp_create_nonce( 'pdl-checkout-' . $this->id ),
        );
    ...
    public function get_payment_notes() {
        ...
        return pdl_get_logs( array( 'object_id' => $this->id, 'object_type' => 'payment' ) );
    }

    public function log( $msg ) {
        return pdl_insert_log( array( 'object_id' => $this->id, 'object_type' => 'payment', 'log_type' => 'payment.note', 'message' => $msg ) );
    }
    ...
    public static function get_stati() {
        $stati = array();
        $stati['pending'] = _x( 'Pending', 'payment', 'PDM' );
        $stati['failed'] = _x( 'Failed', 'payment', 'PDM' );
        $stati['completed'] = _x( 'Completed', 'payment', 'PDM' );
        $stati['canceled'] = _x( 'Canceled', 'payment', 'PDM' );
        $stati['on-hold'] = _x( 'On Hold', 'payment', 'PDM' );
        $stati['refunded'] = _x( 'Refunded', 'payment', 'PDM' );
