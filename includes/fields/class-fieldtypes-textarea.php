...
class PDL_FieldTypes_TextArea extends PDL_Form_Field_Type {

    private $tinymce_settings = array();
    private $quicktags_settings = array();

    public function __construct() {
        parent::__construct( _x('Textarea', 'form-fields api', 'PDM') );

        add_filter( 'pdl_form_field_html_value', array( $this, 'maybe_shorten_output_in_excerpt' ), 10, 4 );
    }
    ...
    private function render_wysiwyg_editor( $field, $value ) {
        ...
        $settings = _WP_Editors::parse_settings( 'pdl-field-' . $field->get_id(), $settings );
        _WP_Editors::editor_settings( 'pdl-field-' . $field->get_id(), $settings );
        ...
        $html = sprintf(
            '<textarea id="%s" class="pdl-editor-area" name="%s">%s</textarea>',
            'pdl-field-' . $field->get_id(),
            'listingfields[' . $field->get_id() . ']',
            $value ? esc_attr( $value ) : ''
        );
        ...
        $html.= sprintf(
            '<script type="text/javascript">
                var PDLTinyMCESettings = PDLTinyMCESettings || {};

                PDLTinyMCESettings[ \'%s\' ] = {
                    \'tinymce\': %s,
                    \'quicktags\': %s
                };
            </script>',
    ...
    public function render_field_settings( &$field=null, $association=null ) {
        ...
        $settings['allow_html'][] = _x( 'Allow HTML input for this field?', 'form-fields admin', 'PDM' );
        $settings['allow_html'][] = '<input type="checkbox" value="1" name="field[allow_html]" ' . ( $field && $field->data( 'allow_html' ) ? ' checked="checked"' : '' ) . ' />';

        $settings['allow_iframes'][] = _x( 'Allow IFRAME tags in content?', 'form-fields admin', 'PDM' );
        $settings['allow_iframes'][] =
            '<div class="iframe-confirm pdl-note warning">' .
            '<p>' . _x( 'Enabling iframe support in your listings can allow users to execute arbitrary scripts on a page if they want, which can possibly infect your site with malware. We do NOT recommend using this setting UNLESS you are posting the listings yourself and have sole control over the content. Are you sure you want to enable this?', 'admin form-fields', 'PDM' ) . '</p>' .
            '<a href="#" class="button no">' . _x( 'No', 'form-fields admin', 'PDM' ) . '</a> ' .
            '<a href="#" class="button button-primary yes">' . _x( 'Yes', 'form-fields admin', 'PDM' ) . '</a>' .
            '</div>' .
        ...
        if ( ( $field && in_array( $field->get_association(), array( 'content', 'excerpt' ) ) ) || ( in_array( $association, array( 'content', 'excerpt' ) ) ) ) {
            $settings['allow_shortcodes'][] = _x( 'Allow WordPress shortcodes in this field?', 'form-fields admin', 'PDM' );
        ...
        if ( ( $field && $field->get_association() == 'content' ) || ( $association == 'content' ) ) {
            $settings['wysiwyg_editor'][] = _x( 'Display a WYSIWYG editor on the frontend?', 'form-fields admin', 'PDM' );
            ...
            $desc = _x( '<b>Warning:</b> Users can use this feature to get around your image limits in fee plans.', 'form-fields admin', 'PDM' );
            $settings['wysiwyg_images'][] = _x( 'Allow images in WYSIWYG editor?', 'form-fields admin', 'PDM' );
            ...
            $desc = _x( '<b>Advanced users only!</b> Unless you\'ve been told to change this, don\'t switch it unless you know what you\'re doing.', 'form-fields admin', 'PDM' );
            $settings['allow_filters'][] = _x( 'Apply "the_content" filter before displaying this field?', 'form-fields admin', 'PDM' );
            ...
            $desc = _x( 'Truncates the description field to the value set here. To display all of the description, set to 0.', 'form-fields admin', 'PDM' );
            $settings['max_length'][] = _x( 'Max length of Description field to use in List (excerpt) view', 'form-fields admin', 'PDM' );
            $settings['max_length'][] = '<input type="number" value="' . ( $field && $field->data( 'max_length' ) ? $field->data( 'max_length' ) : 0 ) . '" name="field[max_length]" /> <span class="pdl-setting-description">' . $desc . '</span>';
        }

        if ( ( $field && $field->get_association() == 'excerpt' ) || ( $association == 'excerpt' ) ) {
            $settings['auto_excerpt'][] = _x( 'Automatically generate excerpt from content field?', 'form-fields admin', 'PDM' );
        ...
        if ( 'content' == $field->get_association() ) {
            if ( $field->data( 'allow_filters' ) ) {
                ...
                if ( $field->data( 'allow_shortcodes' ) ) {
                    ...
                    if ( pdl_get_option( 'disable-cpt' ) || in_array( pdl_current_view(), array( 'search', 'all_listings' ), true ) ) {
