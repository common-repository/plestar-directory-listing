...
class PDL_Form_Field_Type {
    ...
    public function get_id() {
        ...
        $id = str_replace( 'pdl_fieldtypes_', '', $id );
        $id = str_replace( 'pdl_', '', $id );
    ...
    public function get_name() {
        if ( empty( $this->name ) ) {
            $name = get_class( $this );
            $name = str_replace( 'PDL_FieldTypes_', '', $name );
            $name = str_replace( 'PDL_', '', $name );
    ...
    public function get_field_value( &$field, $post_id ) {
        ...
        switch ( $field->get_association() ) {
            ...
            case 'content':
                ...
                break;
            case 'category':
                $value = wp_get_object_terms( $post_id, PDL_CATEGORY_TAX, array( 'fields' => 'ids' ) );
                break;
            case 'tags':
                $value = wp_get_object_terms( $post_id, PDL_TAGS_TAX, array( 'fields' => 'names' ) );
                ...
                break;
            case 'meta':
                $value = get_post_meta( $post_id, '_pdl[fields][' . $field->get_id() . ']', true );
                break;
            default:
                $value = null;
                break;
    ...
    public function get_field_html_value( &$field, $post_id ) {
        ...
        switch ( $field->get_association() ) {
            case 'title':
                ...
                if ( 'show_listing' == pdl_current_view() ) {
                    break;
                }

                $value = sprintf( '<a href="%s" target=%s >%s</a>',
                                  get_permalink( $post_id ),
                                  pdl_get_option( 'listing-link-in-new-tab' ) ? '"_blank" rel="noopener"' : '"_self"',
                                  $value );
                break;
            case 'excerpt':
            ...
            case 'content':
                ...
                break;
            case 'category':
                $value = get_the_term_list( $post_id, PDL_CATEGORY_TAX, '', ', ', '' );
                break;
            case 'tags':
                $value = get_the_term_list( $post_id, PDL_TAGS_TAX, '', ', ', '' );
                break;
            case 'meta':
            default:
    ...
    public function store_field_value( &$field, $post_id, $value ) {
        switch ( $field->get_association() ) {
            ...
            case 'content':
                ...
                break;
            case 'category':
                wp_set_post_terms( $post_id, $value, PDL_CATEGORY_TAX, false );
                break;
            case 'tags':
                wp_set_post_terms( $post_id, $value, PDL_TAGS_TAX, false );
                break;
            case 'meta':
            default:
                update_post_meta( $post_id, '_pdl[fields][' . $field->get_id() . ']', $value );
                break;
        }
    }
    ...
    public function render_field( &$field, $value, $render_context, &$extra=null, $field_settings = array() ) {
        ...
        switch ( $render_context ) {
            case 'search':
                $html .= sprintf( '<div class="pdl-search-filter %s %s" %s>',
                                  $field->get_field_type()->get_id(),
                                  implode(' ', $field->get_css_classes( $render_context ) ),
                                  $this->html_attributes( $field->html_attributes ) );
                $html .= '<div class="pdl-search-field-label">';
                $html .= sprintf( '<label for="%s">%s</label>', 'pdl-field-' . $field->get_id(), esc_html( apply_filters( 'pdl_render_field_label', $field->get_label(), $field ) ) );

                if ( $field->has_validator( 'required-in-search' ) ) {
                    $html .= '<span class="pdl-form-field-required-indicator">*</span>';
                }
                ...
                $field_inner = apply_filters_ref_array( 'pdl_render_field_inner', array( $field_inner, &$field, $value, $render_context, &$extra ) );
            ...
            case 'edit':
            default:
                $html_attributes = $this->html_attributes( apply_filters_ref_array( 'pdl_render_field_html_attributes', array( $field->html_attributes, &$field, $value, $render_context, &$extra ) ) );
                ...
                $html .= sprintf( '<label for="%s">%s</label>', 'pdl-field-' . $field->get_id(), apply_filters( 'pdl_render_field_label', $field->get_label(), $field ) );
                ...
                $field_description = trim( apply_filters( 'pdl_render_field_description', $field->get_description(), $field ) );
                if ( $field_description )
                    $html .= '<div class="pdl-form-field-description">' . $field_description .'</div>';

                $html .= '<div class="pdl-form-field-html pdl-form-field-inner">';
     ...
     * @param object $field the deleted PDL_FormField object.
     */
    public function cleanup( &$field ) {
        if ( $field->get_association() == 'meta' ) {
            global $wpdb;
            $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->postmeta} WHERE meta_key = %s", '_pdl[fields][' . $field->get_id() . ']' ) );
        }
    ...
    public static function standard_display_wrapper( $labelorfield, $content=null, $extra_classes='', $args=array() ) {
        $css_classes = '';
        $css_classes .= 'pdl-field-display pdl-field pdl-field-value field-display field-value ';

        if ( is_object( $labelorfield ) ) {
            if ( $labelorfield->has_display_flag( 'social' ) )
                return $content;

            $css_classes .= 'pdl-field-' . self::normalize_name( $labelorfield->get_label() ) . ' ';
            $css_classes .= 'pdl-field-' . $labelorfield->get_association() . ' ';
            $css_classes .= 'pdl-field-type-' . $labelorfield->get_field_type_id() . ' ';
            $css_classes .= 'pdl-field-association-' . $labelorfield->get_association() . ' ';
            $label = $labelorfield->has_display_flag( 'nolabel' ) ? null : $labelorfield->get_label();
        } else {
            $css_classes .= 'pdl-field-' . self::normalize_name( $labelorfield ) . ' ';
            $label = $labelorfield;
        }
        ...
        if ( $label )
            $html .= '<label>' . esc_html( apply_filters( 'pdl_display_field_label', $label, $labelorfield ) ) . ':</label> ';
    ...
    public static function html_attributes( $attrs, $exceptions = array( 'class' ) ) {
        return pdl_html_attributes( $attrs, $exceptions );
    }
    ...
    public static function normalize_name( $name ) {
        $name = pdl_buckwalter_arabic_transliteration( $name );
...
/**
 * @deprecated Since 3.4.2. Use {@link PDL_Form_Field_Type} instead.
 */
class PDL_FormFieldType extends PDL_Form_Field_Type {}
