 ...
 * @since 3.6.5
 */
class PDL_FieldTypes_Date extends PDL_FieldTypes_TextField {

    public function get_name() {
        return _x( 'Date Field', 'form-fields api', 'PDM' );
    }
    ...
    public function render_field_settings(  &$field = null, $association = null) {
        ...
        foreach ( $this->get_formats() as $format => $data ) {
            $select .= sprintf( '<label><input type="radio" name="field[x_date_format]" value="%s" %s />%s</label><br />',
                                ...
                                sprintf( __( '%s (ex. %s)', 'form-fields api', 'PDM' ), strtoupper( $format ), date( $data['date_format'], $now ) ) );
        }

        $settings = array(
            'date_format' => array( _x( 'Date Format', 'form-fields api', 'PDM' ),
                                    $select )
        );
    ...
    public function setup_validation( $field, $validator, $value ) {
        ...
        $args['messages'] = array( 'incorrect_format' => sprintf( _x( '%s must be in the format %s.', 'date field', 'PDM' ),
                                                                  esc_attr( $field->get_label() ),
                                                                  $this->date_format( $field ) ),
                                   'invalid' => sprintf( _x( '%s must be a valid date.', 'date field', 'PDM' ),
                                                         esc_attr( $field->get_label() ) ) );
        return $args;
    }
