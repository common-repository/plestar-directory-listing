<?php
class PDL_FieldTypes_URL extends PDL_Form_Field_Type {

    public function __construct() {
        parent::__construct( _x( 'URL Field', 'form-fields api', 'PDM' ) );
        add_filter( 'pdl_form_field_css_classes', array( $this, 'css_classes' ), 10, 3 );
    }
    ...
    public function render_field_settings( &$field=null, $association=null ) {
        ...
        $settings['new-window'][] = _x( 'Open link in a new window?', 'form-fields admin', 'PDM' );
        ...
        $settings['nofollow'][] = _x( 'Use rel="nofollow" when displaying the link?', 'form-fields admin', 'PDM' );
    ...
    public function render_field_inner( &$field, $value, $context, &$extra=null, $field_settings = array() ) {
        if ( $context == 'search' ) {
            global $pdl;
            return $pdl->formfields->get_field_type( 'textfield' )->render_field_inner( $field, $value[1], $context, $extra, $field_settings );
        }

        $html  = '';
        $html .= '<div class="pdl-url-field-col">';
        $html .= sprintf( '<span class="sublabel">%s</span>', _x( 'URL:', 'form-fields api', 'PDM' ) );
        ...
        $html .= '<div class="pdl-url-field-col">';
        $html .= sprintf( '<span class="sublabel">%s</span>', _x( 'Link Text (optional):', 'form-fields api', 'PDM' ) );
        $html .= sprintf( '<input type="text" id="%s" name="%s" value="%s" placeholder="" />',
                          'pdl-field-' . $field->get_id() . '-title',
