...
class PDL_Form_Field {
    ...
    public function __construct( $attrs=array() ) {
        ...
        $attrs = apply_filters( 'pdl_form_field_args', $attrs );

        $formfields = PDL_FormFields::instance();

        $this->id = intval( $attrs['id'] );
        $this->shortname = $attrs['shortname'];
        $this->label = $attrs['label'];
        $this->description = $attrs['description'];
        $this->type = is_object( $attrs['field_type'] ) ? $attrs['field_type'] : PDL_FormFields::instance()->get_field_type( $attrs['field_type'] );

        if ( !$this->type )
            throw new Exception( _x( 'Invalid form field type', 'form-fields-api', 'PDM' ) );

/*        if ( !$this->type ) // temporary workaround related to 3.0 upgrade issues (issue #365)
            $this->type = PDL_FormFields::instance()->get_field_type( 'textfield' );*/
        ...
        //     $terms = get_terms( $this->association == 'tags' ? PDL_TAGS_TAX : pdl_categories_taxonomy(), 'hide_empty=0&hierarchical=1' );
        //     $options = array();
        ...
        //     $this->field_data['options'] = $options;
            ...
            // handle some special extra data from previous PDL versions
        ...
        do_action_ref_array( 'pdl_form_field_setup', array( &$this ) );
    }
    ...
    public function get_shortname() {
        ...
        if ( ! $this->label ) {
            ...
            $shortname = PDL_Form_Field_Type::normalize_name( $this->label );
        ...
        if ( $this->id ) {
            global $wpdb;
            $wpdb->update( $wpdb->prefix . 'pdl_form_fields', array( 'shortname' => $shortname ), array( 'id' => $this->id ) );
        }
    ...
    public function shortname_noconflict( $shortname ) {
        ...
        if ( ! $this->id )
            $in_use = (bool) $wpdb->get_var( $wpdb->prepare( "SELECT 1 AS x FROM {$wpdb->prefix}pdl_form_fields WHERE shortname = %s LIMIT 1", $shortname ) );
        else
            $in_use = (bool) $wpdb->get_var( $wpdb->prepare( "SELECT 1 AS x FROM {$wpdb->prefix}pdl_form_fields WHERE shortname = %s AND id != %d LIMIT 1", $shortname, $this->id ) );
        ...
        while ( true ) {
            $check = (bool) $wpdb->get_var( $wpdb->prepare( "SELECT 1 AS x FROM {$wpdb->prefix}pdl_form_fields WHERE shortname = %s LIMIT 1", $shortname . '_' . $n ) );
    ...
    public function remove_display_flag( $flagorflags ) {
        ...
        foreach ( $flagorflags as $flag )
            pdl_array_remove_value( $this->display_flags, $flag );
    }
    ...
    public function get_css_classes( $render_context = '' ) {
        $css_classes = array();
        $css_classes[] = 'pdl-form-field';
        $css_classes[] = 'pdl-form-field-id-' . $this->get_id();
        $css_classes[] = 'pdl-form-field-type-' . $this->get_field_type()->get_id();
        $css_classes[] = 'pdl-form-field-label-' . PDL_Form_Field_Type::normalize_name( $this->get_label() );
        $css_classes[] = 'pdl-form-field-association-' . $this->get_association();

        if ( $this->get_description() )
            $css_classes[] = 'pdl-form-field-has-description';

        foreach ( $this->get_validators() as $validator )
            $css_classes[] = 'pdl-form-field-validate-' . $validator;

        if ( $render_context )
            $css_classes[] = 'pdl-form-field-in-' . $render_context;

        // Add own custom CSS classes.
        $css_classes = array_merge( $css_classes, $this->css_classes );

        return apply_filters( 'pdl_form_field_css_classes', $css_classes, $this, $render_context );
    }
    ...
    public function get_behavior_flags() {
        return apply_filters( 'PDL_Form_Field::get_behavior_flags', $this->type->get_behavior_flags( $this ), $this );
    }
    ...
    public function data( $key=null, $default=null ) {
        ...
        return apply_filters( 'pdl_form_field_data', $res, $key, $this );
    }
    ...
    public function value( $post_id, $raw = false ) {
        ...
        if ( ! $raw )
            $value = apply_filters( 'pdl_form_field_value', $value, $post_id, $this );
    ...
    public function html_value( $post_id, $display_context = 'listing' ) {
        ...
        if ( $value && in_array( 'email', $this->validators, true ) && pdl_get_option('override-email-blocking') ) {
        ...
        return apply_filters( 'pdl_form_field_html_value', $value , $post_id, $this, $display_context );
    }
    ...
    public function plain_value( $post_id ) {
        ...
        return apply_filters( 'pdl_form_field_plain_value', $value, $post_id, $this );
    }
    ...
    public function csv_value( $post_id ) {
        ...
        return apply_filters( 'pdl_form_field_csv_value', $value, $post_id, $this );
    }
    ...
    public function convert_input( $input=null ) {
        $val = apply_filters( 'pdl_form_field_pre_convert_input', null, $input, $this );
    ...
    public function store_value( $post_id, $value ) {
        ...
        do_action_ref_array( 'pdl_form_field_store_value', array( &$this, $post_id, $value ) );
    }

    public function is_empty_value( $value ) {
        return apply_filters( 'pdl_form_field_is_empty_value', $this->type->is_empty_value( $value ), $value, $this );
    }

    public function validate( $value, &$errors=null ) {
        $errors = !is_array( $errors ) ? array() : $errors;

        $validation_api = PDL_FieldValidation::instance();
    ...
    public function display( $post_id, $display_context='listing' ) {
        if ( in_array( 'email', $this->validators, true ) ) {
            if ( ! pdl_get_option('override-email-blocking') )
                return '';
        }
        ...
        $html = apply_filters_ref_array( 'pdl_form_field_display', array( $html, $this, $display_context, $post_id ) );
        return $html;
    }
    ...
    public function render( $value = null, $display_context = 'submit', &$extra = null, $field_settings = array() ) {
        do_action_ref_array( 'pdl_form_field_pre_render', array( &$this, $value, $display_context ) );
    ...
    public function save() {
        ...
        if ( !$this->label || trim( $this->label ) == '' )
            return new WP_Error( 'pdl-save-error', _x('Field label is required.', 'form-fields-api', 'PDM') );

        // If performing a field conversion, make sure the types are compatible.
        if ( $this->id ) {
            $orig_type = $wpdb->get_var( $wpdb->prepare( "SELECT field_type FROM {$wpdb->prefix}pdl_form_fields WHERE id = %d", $this->id ) );
            $new_type = $this->type->get_id();

            if ( $orig_type != $new_type ) {
                if ( 'url' == $new_type || 'image' == $new_type || 'url' == $orig_type || 'image' == $orig_type ) {
                    $this->type = PDL_FormFields::instance()->get_field_type( $orig_type );
                    $error_msg = _x( 'You can\'t change from %2$s field type to the one you wanted--the types are incompatible internally. If you want to switch to a field of type %1$s, delete this current field and create a NEW field of type %1$s instead.', 'form-fields-api', 'PDM' );

                    if( PDL_Listing::count_listings() ) {
                        $error_msg .= '<br/><br/>' . _x( '<strong>WARNING</strong>: If you delete this field, the data from it in existing listings will be deleted as well.', 'form-fields-api', 'PDM' );
                    }

                    $error_msg = sprintf( $error_msg, '<strong>' . strtoupper( $new_type ) . '</strong>', '<strong>' . strtoupper( $orig_type ) . '</strong>' );

                    return new WP_Error( 'pdl-field-error', $error_msg );
                }
            }
        }
        ...
        if ( isset( $_POST['field'] ) ) {
            ...
            do_action_ref_array( 'pdl_form_field_settings_process', array( &$this ) );
        ...
        global $pdl;
        $flags = $pdl->formfields->get_association_flags( $this->association );
        
        if ( in_array( 'unique', $flags ) ) {
            if ( $otherfields = pdl_get_form_fields( 'association=' . $this->association ) ) {
                if ( ( count( $otherfields ) > 1 ) || ( $otherfields[0]->get_id() != $this->id ) ) {
                    return new WP_Error( 'pdl-field-error', sprintf( _x( 'There can only be one field with association "%s". Please select another association.', 'form-fields-api', 'PDM' ), $this->association ) );
                }
            }
        }
      ...
      if ( !in_array( $this->type->get_id(), (array) $pdl->formfields->get_association_field_types( $this->association ) ) ) {
            return new WP_Error( 'pdl-field-error', sprintf( _x( '"%s" is an invalid field type for this association.', 'form-fields-api', 'PDM' ), $this->type->get_name() ) );
      }
        ...
        if ( $this->id ) {
            $wpdb->update( "{$wpdb->prefix}pdl_form_fields", $data, array( 'id' => $this->id ) );
        } else {
            $wpdb->insert( "{$wpdb->prefix}pdl_form_fields", $data );
            $this->id = intval( $wpdb->insert_id );
        }

        wp_cache_delete( $this->id, 'pdl formfields' );
    }
    ...
    public function delete() {
        if ( !$this->id )
            return new WP_Error( 'pdl-delete-error', _x( 'Invalid field ID', 'form-fields-api', 'PDM' ) );

        global $pdl;
        $flags = $pdl->formfields->get_association_flags( $this->association );

        if ( in_array( 'required', $flags ) ) {
            $otherfields = pdl_get_form_fields( array( 'association' => $this->association ) );

            if ( !$otherfields || ( $otherfields[0]->get_id() == $this->id ) )
               return new WP_Error( 'pdl-delete-error', _x( "This form field can't be deleted because it is required for the plugin to work.", 'form-fields api', 'PDM' ) ); 
        }

        global $wpdb;

        if ( $wpdb->query( $wpdb->prepare( "DELETE FROM  {$wpdb->prefix}pdl_form_fields WHERE id = %d", $this->id ) ) !== false ) {
            ...
            return new WP_Error( 'pdl-delete-error', _x( 'An error occurred while trying to delete this field.', 'form-fields-api', 'PDM' ) );
        }
    ...
    public function reorder( $delta=0 ) {
        ...
        if ( $delta > 0 ) {
            $fields = $wpdb->get_results( $wpdb->prepare( "SELECT id, weight FROM {$wpdb->prefix}pdl_form_fields WHERE weight >= %d ORDER BY weight ASC", $this->weight ) );
            ...
            foreach ( $fields as &$f ) {
                $wpdb->update( "{$wpdb->prefix}pdl_form_fields", array( 'weight' => $f->weight ), array( 'id' => $f->id ) );
            }
        } else {
            $fields = $wpdb->get_results( $wpdb->prepare( "SELECT id, weight FROM {$wpdb->prefix}pdl_form_fields WHERE weight <= %d ORDER BY weight ASC", $this->weight ) );
    ...
    public function configure_search( $query, &$search ) {
        ...
        $res = apply_filters_ref_array( 'pdl_pre_configure_search', array( false, $this, $query, $search ) );
        if ( is_array( $res ) )
            return apply_filters_ref_array( 'pdl_configure_search', array( $res, $this, $query, $search ) );
        ...
        if ( is_array( $search_res ) ) {
            $search_res = apply_filters_ref_array( 'pdl_configure_search', array( $search_res, $this, $query, $search ) );
            return $search_res;
        }
        ...
        switch ( $this->get_association() ) {
            ...
            case 'category':
                ...
                $tax = ( 'tags' == $this->get_association() ? PDL_TAGS_TAX : PDL_CATEGORY_TAX );
                $tt_ids = array();
    ...
    /**
     * Creates a PDL_FormField from a database record.
     * @param int $id the database record ID.
     * @return PDL_FormField a valid PDL_FormField if the record exists or null if not.
     */
    public static function get( $id ) {
        ...
        $_field = wp_cache_get( $id, 'pdl formfields' );

        if ( ! $_field ) {
            $_field = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_form_fields WHERE id = %d", $id ) );
            ...
            wp_cache_set( $id, $_field, 'pdl formfields' );
        }

        try {
            return new PDL_FormField( $_field );
        } catch (Exception $e ) {
            return null;
        }
    }
    ...
    public static function find_by_tag( $tag ) {
        ...
        $field_id = absint( $wpdb->get_var( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}pdl_form_fields WHERE tag = %s LIMIT 1", $tag ) ) );
...
/**
 * @deprecated Since 3.4.2. Use {@link PDL_Form_Field} instead.
 */
class PDL_FormField extends PDL_Form_Field {}
