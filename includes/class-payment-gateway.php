...
abstract class PDL__Payment_Gateway {
    ...
    public function is_enabled( $no_errors = true ) {
        $setting_on = pdl_get_option( $this->get_id() );
        if ( ! $no_errors )
            return $setting_on;
    ...
    public function in_test_mode() {
        return pdl_get_option( 'payments-test-mode' );
    }

    public function get_option( $key ) {
        return pdl_get_option( $this->get_id() . '-' . $key );
    }
    ...
    public function render_form( $payment, $errors = array() ) {
        ...
        return pdl_x_render( 'checkout-billing-form', $vars );
    }
    ...
    public function validate_form( $payment ) {
        ...
        foreach ( $required as $req_field ) {
            ...
            if ( ! $field_value ) {
                $errors[ $req_field ] = _x( 'This field is required (' . $req_field . ').', 'payment-gateway', 'PDM' );
            }
        }
    ...
    public function get_listener_url() {
        return add_query_arg( 'pdl-listener', $this->get_id(), home_url( 'index.php' ) );
    }

    public function cancel_subscription( $listing, $subscription ) {
        $message = __( "There was an unexpected error trying to cancel your subscription. Please contact the website's administrator mentioning this problem. The administrator should be able to cancel your subscription contacting the payment processor directly.", 'PDM' );
