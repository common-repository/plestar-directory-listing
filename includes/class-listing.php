...
require_once( PDL_PATH . 'includes/class-listing-subscription.php' );
require_once( PDL_PATH . 'includes/helpers/class-listing-image.php' );
...
class PDL_Listing {
    ...
    public function get_field_value( $id ) {
        ...
        if ( is_numeric( $id ) ) {
            $field = pdl_get_form_field( $id );
        } else {
            $field = pdl_get_form_fields( array( 'association' => $id, 'unique' => true ) );
        }
    ...
    public function get_modified_date() {
        ...
        return pdl_date( get_post_modified_time( 'U', false, $this->id ) );
    }
    ...
    public function get_images( $fields = 'all', $sorted = false ) {
        ...
        foreach ( get_posts( $q ) as $attachment ) {
            ...
            if ( 'id' == $fields || 'ids' == $fields )
                ...
                $result[] = PDL_Listing_Image::get( $attachment->ID );
        }
    ...
    public function get_images_meta() {
        ...
        foreach ( $images as $img_id ) {
            $meta[ $img_id ] = array( 'order' => (int) get_post_meta( $img_id, '_pdl_image_weight', true ),
                                      'caption' => strval( get_post_meta( $img_id, '_pdl_image_caption', true ) ) );
        }
    ...
    public function set_thumbnail_id( $image_id ) {
        if ( ! $image_id )
            return delete_post_meta( $this->id, '_pdl[thumbnail_id]' );

        return update_post_meta( $this->id, '_pdl[thumbnail_id]', $image_id );
    }
    ...
    public function get_thumbnail() {
        $thumbnail_id = get_post_meta( $this->id, '_pdl[thumbnail_id]', true );
    ...
    public function calculate_expiration_date( $time, &$fee ) {
        ...
        } else if ( is_a( $fee, 'PDL__Fee_Plan' ) ) {
            $days = $fee->days;
    ...
    public function get_categories( $fields='all' ) {
        ...
        return wp_get_post_terms( $this->id, PDL_CATEGORY_TAX, $args );
    }

    public function set_categories( $categories ) {
        $category_ids = array_map( 'intval', $categories );
        wp_set_post_terms( $this->id, $category_ids, PDL_CATEGORY_TAX, false );
    }
    ...
    public function get_subscription() {
        try {
            $subscription = new PDL__Listing_Subscription( $this->id );
        } catch ( Exception $e ) {
            $subscription = null;
        }
        return $subscription;
    ...
    public function has_subscription() {
        global $wpdb;
        return absint( $wpdb->get_var( $wpdb->prepare( "SELECT COUNT(*) FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d AND is_recurring = %d", $this->id, 1 ) ) ) > 0;
    }
    ...
    public function get_payment_status() {
        ...
        if ( PDL_Payment::objects()->filter( array( 'listing_id' => $this->id, 'status' => 'pending' ) )->count() > 0 )
            $status = 'pending';

        return apply_filters( 'PDL_Listing::get_payment_status', $status, $this->id );
    }
    ...
    public function get_renewal_date() {
        ...
        $payments = PDL_Payment::objects()->filter( $filters )->order_by( '-id' )->to_array();

        if ( ! isset( $payments[0] ) ) {
            return null;
        }

        return pdl_date_full_format( strtotime( $payments[0]->created_at ) );
    }
    ...
    public function get_payments() {
        $payments = PDL_Payment::objects()->filter( array( 'listing_id' => $this->id ) );
        return $payments;
    }

    public function get_latest_payments() {
        return PDL_Payment::objects()->filter( array( 'listing_id' => $this->id ) )->order_by( '-id' )->to_array();
    }
    ...
    public function set_status( $status ) {
        ...
        $wpdb->update( $wpdb->prefix . 'pdl_listings', array( 'listing_status' => $new_status ), array( 'listing_id' => $this->id ) );
        ...
        case 'expired':
            ...
            pdl_insert_log( array( 'log_type' => 'listing.expired', 'object_id' => $this->id, 'message' => _x( 'Listing expired', 'listing', 'PDM' ) ) );
            do_action( 'pdl_listing_expired', $this );
            break;
        default:
            break;
        }

        do_action( 'pdl_listing_status_change', $this, $old_status, $new_status );
    }
    ...
    public function delete() {
        global $wpdb;
        $wpdb->update( $wpdb->posts, array( 'post_status' => pdl_get_option( 'deleted-status' ) ), array( 'ID' => $this->id ) );
    ...
    public function notify( $kind = 'save', &$extra = null ) {
        ...
        //
        //     case 'edit':
        //         do_action_ref_array( 'pdl_edit_listing', array( &$this, &$extra ) );
        //         break;
        //
        //     default:
        //         break;
    ...
    public function renew() {
        ...
        if ( ! empty( $row ) ) {
            $wpdb->update( $wpdb->prefix . 'pdl_listings', $row, array( 'listing_id' => $this->id ) );
        } else {
            $wpdb->query( $wpdb->prepare( "UPDATE {$wpdb->prefix}pdl_listings SET expiration_date = NULL WHERE listing_id = %d", $this->id ) );
        }

        $this->set_status( 'complete' );
        $this->set_post_status( 'publish' );

        do_action( 'pdl_listing_renewed', $this, false, 'admin' );
    }
    ...
    public function get_renewal_url( $deprecated = 0 ) {
        ...
        return pdl_url( 'renew_listing', $this->id );
    }
    ...
    public function get_access_key() {
        if ( $key = get_post_meta( $this->id, '_pdl[access_key]', true ) )
            return $key;

        // Generate access key.
        $new_key = sha1( sprintf( '%s%s%d', AUTH_KEY, uniqid( '', true ), rand( 1, 1000 ) ) );
        if ( update_post_meta( $this->id, '_pdl[access_key]', $new_key ) )
            return $new_key;
    }
    ...
    public function get_sticky_status( $consider_plans = true ) {
        ...
        $is_sticky = (bool) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT 1 AS x FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d AND is_sticky = %d",
                $this->id,
                1 )
        );
    ...
    public function get_fee_plan() {
        ...
        $res = $wpdb->get_row( $wpdb->prepare( "SELECT listing_id, fee_id, fee_price, fee_days, fee_images, expiration_date, is_recurring, is_sticky FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d LIMIT 1", $this->id ) );
        if ( ! $res )
            return false;

        if ( $res->fee_id )
            $fee = pdl_get_fee_plan( $res->fee_id );
        else
            $fee = null;

        $res->fee = $fee;
        $res->fee_label = $fee ? $fee->label : _x( '(Unavailable Plan)', 'listing', 'PDM' );
    ...
    public function update_plan( $plan = null, $args = array() ) {
        ...
        if ( is_numeric( $plan ) || ( is_array( $plan ) && ! empty( $plan['fee_id'] ) ) ) {
            ...
            if ( $plan_ = pdl_get_fee_plan( $plan_id ) ) {
                $row['fee_id'] = $plan_id;
        ...
        if ( ! $args['clear'] ) {
            $old_row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d", $this->id ), ARRAY_A );
        ...
        return $wpdb->replace( "{$wpdb->prefix}pdl_listings", $row );
    }
    ...
    public function set_fee_plan( $fee, $recurring_data = array() ) {
        ...
        if ( is_null( $fee ) ) {
            $wpdb->delete( $wpdb->prefix . 'pdl_listings', array( 'listing_id' => $this->id ) );
            // $wpdb->replace( $wpdb->prefix . 'pdl_listings', array( 'listing_id' => $this->id, 'fee_id' => null, 'fee_days' => 0, 'fee_images' => 0, 'is_sticky' => 0, 'expiration_date' => null ) );
            return true;
        }

        $fee = is_numeric( $fee ) ? pdl_get_fee_plan( $fee ) : $fee;
        ...
        $row =  array( 'listing_id' => $this->id,
                       ...
                       'fee_price' => $fee->calculate_amount( wp_get_post_terms( $this->id, PDL_CATEGORY_TAX, array( 'fields' => 'ids' ) ) ),
    ...
    public function generate_or_retrieve_payment() {
        ...
        $existing_payment = PDL_Payment::objects()->filter( array( 'listing_id' => $this->id, 'payment_type' => 'initial' ) )->get();
    ...
    private function create_payment_from_plan( $payment_type, $plan ) {
        $payment = new PDL_Payment( array(
            'listing_id' => $this->id,
            'payment_type' => $payment_type,
        ) );

        if ( $plan->is_recurring ) {
            $item_description = sprintf( _x( 'Plan "%s" (recurring)', 'listing', 'PDM' ), $plan->fee_label );
        } else {
            $item_description = sprintf( _x( 'Plan "%s"', 'listing', 'PDM' ), $plan->fee_label );
        }
    ...
    public function get_status( $force_refresh = false, $calculate = true ) {
        ...
        $status_ = $wpdb->get_var( $wpdb->prepare( "SELECT listing_status FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d", $this->id ) );
        ...
        $status = apply_filters( 'pdl_listing_status', $status, $this->id );

        if ( ! $status_ || $status_ != $status || $force_refresh )
            $wpdb->update( $wpdb->prefix . 'pdl_listings', array( 'listing_status' => $status ), array( 'listing_id' => $this->id ) );
    ...
    private function calculate_status() {
        ...
        $is_expired = (bool) $wpdb->get_var(
            $wpdb->prepare(
                "SELECT 1 AS x FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d AND expiration_date IS NOT NULL AND expiration_date < %s",
                $this->id,
                current_time( 'mysql' )
            )
        );
        $pending_payment = $wpdb->get_row(
            $wpdb->prepare(
                "SELECT * FROM {$wpdb->prefix}pdl_payments WHERE listing_id = %d AND status = %s ORDER BY id DESC LIMIT 1",
                $this->id,
                'pending'
            )
        );
    ...
    public static function get_stati() {
        $stati = array(
            'unknown' => _x( 'Unknown', 'listing status', 'PDM' ),
            'legacy' => _x( 'Legacy', 'listing status', 'PDM' ),
            'incomplete' => _x( 'Incomplete', 'listing status', 'PDM' ),
            'pending_payment' => _x( 'Pending Payment', 'listing status', 'PDM' ),
            'complete' => _x( 'Complete', 'listing status', 'PDM' ),
            'pending_upgrade' => _x( 'Pending Upgrade', 'listing status', 'PDM' ),
            'expired' => _x( 'Expired', 'listing status', 'PDM' ),
            'pending_renewal' => _x( 'Pending Renewal', 'listing status', 'PDM' ),
            'abandoned' => _x( 'Abandoned', 'listing status', 'PDM' ),
        );
        $stati = apply_filters( 'pdl_listing_stati', $stati );
    ...
    public static function count_listings( $args = array() ) {
        ...
        $query = "SELECT COUNT(*) FROM {$wpdb->posts} p JOIN {$wpdb->prefix}pdl_listings l ON p.ID = l.listing_id WHERE p.post_type = %s AND p.post_status IN ({$query_post_statuses}) AND l.listing_status IN ({$query_listing_statuses})";
        $query = $wpdb->prepare( $query, PDL_POST_TYPE );
    ...
    private static function parse_count_args( $args = array() ) {
        ...
        if ( ! is_array( $args['status'] ) ) {
            if ( 'all' == $args['status'] ) {
                $args['status'] = array_keys( PDL_Listing::get_stati() );
            } else {
    ...
    public static function count_listings_with_no_fee_plan( $args = array() ) {
        ...
        $query.= "LEFT JOIN {$wpdb->prefix}pdl_listings l ON ( p.ID = l.listing_id ) ";
        $query.= "WHERE p.post_type = %s ";
        $query.= "AND post_status IN ({$query_post_statuses}) ";
        $query.= "AND l.listing_id IS NULL ";

        return absint( $wpdb->get_var( $wpdb->prepare( $query, PDL_POST_TYPE ) ) );
    }
    ...
    public function get_sequence_id() {
        $sequence_id = get_post_meta( $this->id, '_pdl[import_sequence_id]', true );
        ...
        if ( ! $sequence_id ) {
            ...
            $candidate = intval( $wpdb->get_var( $wpdb->prepare( "SELECT MAX(CAST(meta_value AS UNSIGNED INTEGER )) FROM {$wpdb->postmeta} WHERE meta_key = %s",
                                                                 '_pdl[import_sequence_id]' ) ) );
            $candidate++;

            if ( false == add_post_meta( $this->id, '_pdl[import_sequence_id]', $candidate, true ) )
                $sequence_id = 0;
            else
    ...
    public function get_flags() {
        ...
        $flags = trim( $wpdb->get_var( $wpdb->prepare( "SELECT flags FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d", $this->id ) ) );
    ...
    public function set_flag( $flag ) {
        ...
        $wpdb->update( $wpdb->prefix . 'pdl_listings', array( 'flags' => implode( ',', $flags ) ), array( 'listing_id' => $this->id ) );
    }
    ...
    public function _after_save( $context = '' ) {
        if ( 'submit-new' == $context ) {
            do_action( 'PDL_Listing::listing_created', $this->id );
            do_action( 'pdl_add_listing', $this->id );
        } elseif ( 'submit-edit' == $context ) {
            do_action( 'pdl_edit_listing', $this->id );
            do_action( 'PDL_Listing::listing_edited', $this->id );
        }

        do_action( 'pdl_save_listing', $this->id, 'submit-new' == $context );
    ...
    public function after_delete( $context = '' ) {
        ...
        // Remove listing fees.
        $wpdb->query( $wpdb->prepare( "DELETE FROM {$wpdb->prefix}pdl_listings WHERE listing_id = %d", $this->id ) );

        // Delete logs.
        $wpdb->delete( $wpdb->prefix . 'pdl_logs', array( 'object_type' => 'listing', 'object_id' => $this->id ) );

        // Remove payment information.
        foreach ( $wpdb->get_col( $wpdb->prepare( "SELECT id FROM {$wpdb->prefix}pdl_payments WHERE listing_id = %d", $this->id ) ) as $payment_id ) {
    ...
    public static function get( $id ) {
        if ( PDL_POST_TYPE !== get_post_type( $id ) )
            return null;
