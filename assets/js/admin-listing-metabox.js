jQuery( function( $ ) {
    ...
    $( '#pdl-listing-metabox-plan-info input[name="listing_plan[expiration_date]"]' ).datepicker({
        dateFormat: 'yy-mm-dd',
    ...
    var $metabox_tab = $('#pdl-listing-metabox-plan-info');
    ...
    var updateText = function() {
        ...
        var $plan = $('select#pdl-listing-plan-select').find('option[value="' + plan_id + '"]');
        if ( $plan.length > 0) {
            var plan_data = $.parseJSON($plan.attr('data-plan-info'));

            $('#pdl-listing-plan-prop-label').html(
                pdlListingMetaboxL10n.planDisplayFormat.replace('{{plan_id}}', plan_data.id)
                                                         .replace('{{plan_label}}', plan_data.label));
            $( '#pdl-listing-plan-prop-amount' ).html( plan_data.amount ? plan_data.amount : '-' );
            $( '#pdl-listing-plan-prop-is_sticky' ).html( plan_data.sticky ? pdlListingMetaboxL10n.yes : pdlListingMetaboxL10n.no );
            $( '#pdl-listing-plan-prop-is_recurring' ).html( plan_data.recurring ? pdlListingMetaboxL10n.yes : pdlListingMetaboxL10n.no );
        }

        $('#pdl-listing-plan-prop-expiration').html(expiration ? expiration : pdlListingMetaboxL10n.noExpiration);
        $('#pdl-listing-plan-prop-images').html(images);
    };
    ...
    $metabox_tab.find('.value-editor a.update-value, .value-editor a.cancel-edit').click(function(e) {
        ...
        if ( $(this).is( '.cancel-edit' ) ) {
            ...
            if ($input.is('#pdl-listing-plan-select')) {
