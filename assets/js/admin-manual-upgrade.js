jQuery(function($) {
    var $startButton = $('.pdl-admin-page-manual-upgrade a.start-upgrade');
    var $pauseButton = $('.pdl-admin-page-manual-upgrade a.pause-upgrade');
    ...
    var makeProgress = function() {
        ...
        var data = { action: 'pdl-manual-upgrade' };
    ...
    // Migration specific.
    $( '#pdl-manual-upgrade-18_0-config #add-fee-form form#pdl-fee-form' ).submit(function(e) {
        e.preventDefault();
    ...
    $( '#pdl-manual-upgrade-18_0-config select.level-migration' ).change(function(e) {
        ...
        if ( 'create' == selection.val() ) {
            $( 'form#pdl-fee-form' ).get(0).reset();
            $( 'form#pdl-fee-form' ).data( 'levelId', $( this ).attr( 'data-level-id' ) );
            $config.find( '.new-fee-summary tbody td' ).html( '-' );
            tb_show( $( '#add-fee-form' ).attr( 'data-title' ), '#TB_inline?inlineId=add-fee-form' );
            $( 'form#pdl-fee-form input:first' ).focus();
        }
    });
